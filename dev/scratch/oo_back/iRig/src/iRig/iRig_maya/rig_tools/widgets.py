...
import tex_utils  # Needed for IO_Widget

import rig_tools.frankenstein.ui as rig_f_ui
...
class PanelGrid(tool_panel.GridWidget):
    def __init__(self, parent=None):
        super(PanelGrid, self).__init__(parent)

        # Vars
        self.divider_fn = "import icon_api.utils as i_utils;i_utils.error('This is a right-click divider, not a build option.', dialog=True)"
        self.rcl_only_fn = "import icon_api.utils as i_utils;i_utils.error('Only right-click options are available. Please choose one of those.', dialog=True)"
        self.rcl_overlay = "rig/rig_overlay_rightclick.png"
        ugroups = os.environ.get("TT_USERGROUPS")
        if "Leads" in ugroups and ("Pipeline" in ugroups or "Rigging" in ugroups):
            self.is_lead = True
        else:
            self.is_lead = False
        self.projcode = os.environ.get("TT_PROJCODE")
        ...
        frankenstein_lyt, frankenstein_grp = self._create_section(title="Frankenstein")
        # Setup / Templates
        self._button_setup(layout=frankenstein_lyt, row=0, column=0)
        # Packs
        self._button_frankenstein(layout=frankenstein_lyt, row=0, column=1)
        # Faces
        self._button_faces(layout=frankenstein_lyt, row=0, column=2)
        # Hacky Legacy Things
        self._button_hack(layout=frankenstein_lyt, row=0, column=3)
        # Import / Export
        self._button_io_import_export(layout=frankenstein_lyt, row=0, column=4)
        # Documentation
        self._button_documentation(layout=frankenstein_lyt, row=0, column=5)
        # # Spacers
        # self._add_spacers(layout=frankenstein_lyt, row=0, columns=[5])

        ##################################### Controls Section
        controls_lyt, controls_grp = self._create_section(title="Controls")
        # Create Controls
        self._button_controls(layout=controls_lyt, row=0, column=0)
        # Save Shape
        self._button_save_shape(layout=controls_lyt, row=0, column=1)
        # Zeroed Top Group
        self._button_zeroed_group(layout=controls_lyt, row=0, column=2)
        # Control Color
        self._button_control_color(layout=controls_lyt, row=0, column=3)
        # Map Names
        self._button_map_controls(layout=controls_lyt, row=0, column=4)
        # Mirror Controls
        self._button_mirror_controls(layout=controls_lyt, row=0, column=5)
        # Scale Slider
        sub_controls_lyt, sub_controls_grp = self._create_section(as_grid=False)
        self._widget_control_slider(layout=sub_controls_lyt)

        ##################################### Attributes Section
        attributes_lyt, attributes_grp = self._create_section(title="Attributes")
        # Multi CB
        self._button_multi_channelbox(layout=attributes_lyt, row=0, column=0)
        # Lock/Unlock
        self._button_lock_unlock_attrs(layout=attributes_lyt, row=0, column=1)
        # Quick Vis
        self._button_quick_vis(layout=attributes_lyt, row=0, column=2)
        # Attr Up
        self._button_attr_up(layout=attributes_lyt, row=0, column=3)
        # Attr Down
        self._button_attr_down(layout=attributes_lyt, row=0, column=4)
        # Promote Attr
        self._button_promote_attr(layout=attributes_lyt, row=0, column=5)
        # Update Defaults
        self._button_update_attrs(layout=attributes_lyt, row=1, column=0)
        # Reset to Defaults
        self._button_reset_attrs(layout=attributes_lyt, row=1, column=1)
        # Add Follow Attr
        self._button_follow_attr(layout=attributes_lyt, row=1, column=2)
        # Spacers
        self._add_spacers(layout=attributes_lyt, row=1, columns=[3, 4, 5])

        ##################################### Dynamics Section
        dynamics_lyt, dynamics_grp = self._create_section(title="Dynamics")
        # Create Dynamics
        self._button_dynamics(layout=dynamics_lyt, row=0, column=0)
        # Paint Dynamics
        self._button_paint_dynamics(layout=dynamics_lyt, row=0, column=1)
        # Dynamics weights Import Export
        self._button_dynamics_weights_import_export(layout=dynamics_lyt, row=0, column=2)
        # Interactive Playback
        self._button_interactive_playback(layout=dynamics_lyt, row=0, column=3)
        # Spacers
        self._add_spacers(layout=dynamics_lyt, row=0, columns=[4, 5])

        ##################################### Geometry Section
        geometry_lyt, geometry_grp = self._create_section(title="Geometry")
        # Color
        self._button_geometry_color(layout=geometry_lyt, row=0, column=0)
        # Geo Overrides
        self._button_geometry_overrides(layout=geometry_lyt, row=0, column=1)
        # Import > Reference
        self._button_import_references(layout=geometry_lyt, row=0, column=2)
        # Fix Shape Names
        self._button_fix_geometry(layout=geometry_lyt, row=0, column=3)
        # Proxy Geo
        self._button_proxy_geo(layout=geometry_lyt, row=0, column=4)
        # Spacers
        self._add_spacers(layout=geometry_lyt, row=0, columns=[5])

        ##################################### Joints Section
        joints_lyt, joints_grp = self._create_section(title="Joints")
        # Joint Insert Reskin
        self._button_joint(layout=joints_lyt, row=0, column=0)
        # Skinning Tools
        self._button_skinning(layout=joints_lyt, row=0, column=1)
        # Mirror Joint
        self._button_mirror_joint(layout=joints_lyt, row=0, column=2)
        # Select Bind Joints
        self._button_select_bind_joints(layout=joints_lyt, row=0, column=3)
        # Bone On
        self._button_bone_on(layout=joints_lyt, row=0, column=4)
        # Bone Off
        self._button_bone_off(layout=joints_lyt, row=0, column=5)
        # Skinning Tools 2.0
        self._button_skinning_tools(layout=joints_lyt, row=1, column=0)

        ##################################### Deformers Section
        deformers_lyt, deformers_grp = self._create_section(title="Deformers")
        # Box
        self._button_box_lattice(layout=deformers_lyt, row=0, column=0)
        # Create Deformer
        self._button_create_deformer(layout=deformers_lyt, row=0, column=1)
        # Follicles
        self._button_follicles(layout=deformers_lyt, row=0, column=2)
        # Set Smoothing
        self._button_set_smooth(layout=deformers_lyt, row=0, column=3)
        # Paint Soft Cluster
        self._button_paint_softcluster(layout=deformers_lyt, row=0, column=4)
        # Multi Smooth
        self._button_multi_smooth(layout=deformers_lyt, row=0, column=5)

        ##################################### XForm Section
        xform_lyt, xform_grp = self._create_section(title="XForm")
        # At Center
        self._button_at_center(layout=xform_lyt, row=0, column=0)
        # Apply Angle
        self._button_apply_angle(layout=xform_lyt, row=0, column=1)
        # Snap
        self._button_snap(layout=xform_lyt, row=0, column=2)
        # Connect
        self._button_connect(layout=xform_lyt, row=0, column=3)
        # Constrain
        self._button_constrain(layout=xform_lyt, row=0, column=4)
        # Mirror Objects
        self._button_mirror_objects(layout=xform_lyt, row=0, column=5)

        ##################################### Misc Section
        misc_lyt, misc_grp = self._create_section(title="Misc")
        # Find Rigs
        self._button_find_rigs(layout=misc_lyt, row=0, column=0)
        # Rename Tool
        self._button_rename(layout=misc_lyt, row=0, column=1)
        # Add / Remove Sets
        self._button_add_remove_sets(layout=misc_lyt, row=0, column=2)
        # Fix Misc
        self._button_fix_misc(layout=misc_lyt, row=0, column=3)
        # Fix Misc
        self._button_import_calethestics_test_misc(layout=misc_lyt, row=0, column=4)
        # Fix Misc
        self._flo_three(layout=misc_lyt, row=0, column=5)
        # Spacers
        self._add_spacers(layout=misc_lyt, row=0, columns=[4, 5])
    ...
    def _button_setup(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        left-click: Creates a master control template at the center of the scene
        right-click: Create dictionary of rig templates to the classes of the modules in the rig templates directory

        :param layout: (QGridLayout) - creates one row and one column initially, and will expand when new items are inserted
        :param row: (int) - number of rows in the widget
        :param column: (int) - number of columns in the widget
        :return: None
        """
        rcl = collections.OrderedDict()
        for tpt_type in ["Character", "Prop", "Set", "Vehicle"]:
            rcl[
                tpt_type] = "import rig_tools.frankenstein.utils;rig_tools.frankenstein.utils.create_simple_template('{0}')".format(
                tpt_type)

        # create a test character class for leads
        if self.is_lead:
            rcl["TEST: Character"] = "import rig_tools.frankenstein.utils;rig_tools.frankenstein.utils.test_basic()"

        button = tool_panel.IconButton(icon="rig/rig_setup.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.frankenstein.utils;rig_tools.frankenstein.utils.create_master_center_of_geo()",
                                       hotkey_label="Setup and Templates",  # No hotkey, but needed for help button
                                       tool_tip="Create setup and templates.\nLeft-Click: Character Setup\nRight-Click: Other options",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.gold,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_frankenstein(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        left-click: Runs ui.py which creates rigs using the Frankenstein templates, maya/scripts/rig_tools/frankenstein/ui
        right-click: Creates dictionary of selection options to the classes of the modules in the rig pack selection (must be built using Frankenstein)
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Select Info Node (from selected build obj)"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.sel_from_pack_obj('pack_info_node')"
        rcl[
            "Select Bind Joints (from selected build obj)"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.sel_from_pack_obj('bind_joints')"
        rcl[
            "Select Controls (from selected build obj)"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.sel_from_pack_obj('created_controls')"
        if self.is_lead:
            rcl[
                "TEST: Print Joint Pos"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils._get_scene_pack_positions(joint_only=True)"
            rcl[
                "TEST: Biped"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_template('Biped')"
            rcl[
                "TEST: Biped + Geo"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_template('Biped', with_geo=True)"
            rcl[
                "TEST: Biped - Alt"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_template('Biped_Alt')"
            rcl[
                "TEST: Quadruped"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_template('Quadruped')"
            rcl[
                "TEST: Quad + Geo"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_template('Quadruped', with_geo=True)"
            rcl[
                "TEST: Geo for sel packs"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_add_geo()"
            rcl[
                "TEST: Geo for ALL packs"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_add_geo(all_packs=True)"
            rcl[
                "HACK: Referenced packs to Fresh packs"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.test_repo_packjoints_from_ref()"
        button = tool_panel.IconButton(icon="rig/rig_packs.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, rig_f_ui.Frankenstein_Widget,
                                                       "rig/rig_pack_recon.png",
                                                       self.pipedocs_url + "rig_frankenstein/"),
                                       hotkey_label="Open Frankenstein",  # No hotkey, but needed for help button
                                       tool_tip="Open Frankenstein",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.blue_dark,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_faces(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        left-click: Does not create anything in the scene. Brings up a pop-window stating button only has right-click functionality.
        right-click: Create dictionary of face rig templates to the classes of the modules in the face rig templates directory. Includes legacy face rig templates.
        ...
        """
        rcl = collections.OrderedDict()
        rcl["-- Watson (Gen) --"] = self.divider_fn
        rcl["Build Watson Face"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_watson_face(action='build')"
        rcl["Store Slider Poses"] = "import rig_tools.utils.controls as rig_controls;rig_controls.an_mo_auto()"
        rcl["-- KNG --"] = self.divider_fn
        rcl[
            "Face Template - KNG"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_watson_face(action='template', face_type='KNG')"
        rcl[
            "Face Bake - KNG"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_watson_face(action='bake', face_type='KNG')"
        rcl["-- SMO --"] = self.divider_fn
        rcl[
            "Face Template - SMO"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_watson_face(action='template')"
        rcl["Face Bake - SMO"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_watson_face(action='bake')"
        rcl[
            "Proxy Eyes - SMO (select eye geos)"] = "import rig_tools.utils.geometry as rig_geometry;rig_geometry.proxy_eyes_smo(dialog_error=True)"
        rcl[
            "Face Bake: Check Bake Type"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_check_face_bake_type()"
        rcl[
            "Face Bake: Set to KNG"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_specify_face_bake('KNG')"
        rcl[
            "Face Bake: Set to SMO"] = "import rig_tools.utils.face as rig_face;rig_face.legacy_specify_face_bake('SMO')"
        rcl["Control Scale Up"] = "import rig_tools.utils.face as rig_face;rig_face.smo_face_control_scale()"
        rcl["-- TTM --"] = self.divider_fn
        rcl["Face Connect"] = "import rig_tools.utils.face as rig_face;rig_face.ttm_groups()"
        rcl["Face BSH (sel face geo)"] = "import rig_tools.utils.face as rig_face;rig_face.ttm_bsh()"
        rcl["-- Frankenstein --"] = self.divider_fn
        rcl["Face Bake"] = "import rig_tools.utils.face as rig_face;fb = rig_face.FaceBake();fb.run()"
        rcl["Face Bake - Advanced UI"] = partial(tool_panel.PopOff, None, FaceBake_Widget)
        rcl["-- EAV --"] = self.divider_fn
        rcl[
            "Proxy Eyes - EAV"] = "import rig_tools.utils.geometry as rig_geometry;rig_geometry.proxy_eyes_eav(dialog_error=True)"
        rcl[
            "Eyelash 1: Dupe and Attach Curves (sel one edge row)"] = "import rig_tools.utils.nodes as rig_nodes;rig_nodes.dupe_attach_curves_sel()"
        rcl[
            "Eyelash 2: Loft, Foll, Joints (sel attach curves)"] = "import rig_tools.utils.face as rig_face;rig_face.eav_eyelash_setup()"
        rcl[
            "Eyelash 3: Controls (sel follicles)"] = "import rig_tools.utils.face as rig_face;rig_face.eav_eyelash_controls()"
        rcl[
            "Face Loc - Sel Locs"] = "import rig_tools.utils.face as rig_face;rig_face.eav_controls_create(locators=None, do_create_locators=False)"
        rcl[
            "Face Loc - MAKE Locs"] = "import rig_tools.utils.face as rig_face;rig_face.eav_controls_create(locators=None, do_create_locators=True)"
        button = tool_panel.IconButton(icon="rig/rig_faces.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=self.rcl_only_fn,
                                       hotkey_label="Face Rigs",  # No hotkey, but needed for help button
                                       tool_tip="Create face rigs. Right-click: All options to Create Face Rigs",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.rose,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_hack(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        restructure the Frankenstein-built rig to work for Legacy
        left-click: Does not create anything in the scene. Brings up a pop-window stating button only has right-click functionality.
        right-click: Create dictionary of hack options to the classes of the modules in Frankenstein's utils.py and rig_util's utils.py
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Watson Fixes for Pipe"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.legacy_watson_fix_for_current_pipe()"
        rcl["Open Scene Tester"] = "import rig_tools.utils.misc as rig_misc;rig_misc.legacy_scene_tester()"
        rcl["Remove Containers"] = "import rig_tools.utils.nodes as rig_nodes;rig_nodes.remove_node_containers()"
        rcl["LH AI Attrs on Controls"] = "import icon_api.node as i_node;i_node.remove_ai_attrs_sel()"
        rcl[
            "Make Frank know Watson"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.legacy_add_watson_info_attrs()"
        rcl["--- SMO ---"] = self.divider_fn
        rcl[
            "Clean Head Groups"] = "import rig_tools.frankenstein.utils as rig_frankenstein_utils;rig_frankenstein_utils.legacy_clean_head_groups()"
        rcl["Add Jaw Squash"] = "import rig_tools.utils.face as rig_face;rig_face.smo_jaw_squash_add()"
        button = tool_panel.IconButton(icon="rig/rig_hack.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=self.rcl_only_fn,
                                       hotkey_label="Hacky",  # No hotkey, but needed for help button
                                       tool_tip="Hack Things for Legacy to work with Frankenstein",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       # icon_rgb=tool_panel.OverlayColors.blue_mid,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_io_import_export(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button
        Runs IO_Widget which imports and exports data, class in current .py file
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_io_import_export.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, IO_Widget, None,
                                                       self.pipedocs_url + "rig_ios/"),
                                       hotkey_label="Import Export",  # No hotkey, but needed for help button
                                       tool_tip="Import and Export Data",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.orange,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_documentation(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        left-click: opens webrowser to ICON Rigging Documentation Hub webpage
        right-click: create a dictionary of links to the ICON Rigging Documentation Hub webpage or the ICON PipeDocs Rigging Pipeline Tools webpage
        ...
        # if self.is_lead:  # Doesn't work the moment there's a reload, even if run this again
        #     RIG_LOG_cmd = "import rig_tools;from rig_tools import RIG_LOG;RIG_LOG.level ="
        #     RIG_F_LOG_cmd = "import rig_tools.frankenstein;from rig_tools.frankenstein import RIG_F_LOG;RIG_F_LOG.level ="
        #     API_LOG_cmd = "import icon_api;from icon_api import API_LOG;API_LOG.level ="
        #     rcl["Turn on Max Verbose"] = "%s 10;%s 10;%s 10" % (API_LOG_cmd, RIG_LOG_cmd, RIG_F_LOG_cmd)
        #     rcl["Reset Level Verbose"] = "%s 20;%s 20;%s 20" % (API_LOG_cmd, RIG_LOG_cmd, RIG_F_LOG_cmd)

        button = tool_panel.IconButton(icon="rig/rig_docs.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       # Left-click opens Code Formatting Guidelines
                                       command=url_fn('http://hub.icon.local/documentation/docs/rigging/'),
                                       hotkey_label="Open Documentations",  # No hotkey, but needed for help button
    ...
    def _button_controls(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates and adds a button with left and right click functionality
        left-click: runs CreateControl_Widget which creates controls, class in current .py file
        right-click: create a dictionary of functions that edits or retrieves data from the current controls in the scene
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Replace Shape (UI)"] = partial(tool_panel.PopOff, None, ReplaceControlShape_Widget)
        rcl["Add Gimbal (Sel Driver)"] = "import icon_api.control as i_control;i_control.create_gimbal_sel()"
        rcl[
            "Remove Gimbal (Sel Gimbal or Driver)"] = "import icon_api.control as i_control;i_control.remove_gimbal_sel()"
        rcl[
            "Print Number of Cvs (Sel Curve(s))"] = "import rig_tools.utils.controls as rig_controls;rig_controls.get_cv_sel()"
        rcl[
            "Select All Controls"] = "import rig_tools.utils.controls as rig_controls;rig_controls.select_all_controls()"
        rcl["Sample All Control Types"] = "import icon_api.control as i_control;i_control.sample_all_curve_types()"
        button = tool_panel.IconButton(icon="rig/rig_controls.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, CreateControl_Widget),
                                       hotkey_label="Create controls",  # No hotkey, but needed for help button
                                       tool_tip="Create controls",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.blue_light,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_save_shape(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button and runs SaveControlShape_Widget which saves a shape to a .json file (?)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_save_shape.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, SaveControlShape_Widget),
                                       hotkey_label="Save control shape",  # No hotkey, but needed for help button
                                       tool_tip="Save control shape to be available for everyone to create",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.green,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_zeroed_group(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: creates two groups, 'offset' and 'cns'
        --- the first group ('offset') copies the object's attributes
        --- the second group ('cns') parented under the 'offset' group and zeroed out
        --- the selected object is then parented under the 'cns' group and is zeroed out
        right-click: create a dictionary of functions to create an 'offset' zeroed out group or runs AddTopGroup_Widget
        ---AddTopGroup_Widget creates a zeroed top group or takes selection and searches the group that match the string in the 'search' text box and replaces it with the 'replace' text box

        # NEEDS REWORK.
        Almost works. Needs to check if the parent obj has an offset, then create zeroed group based on the parent obj's offset (from it's parent) and child obj's offset
        OR freeze the transforms if the object is a descendant

        Eg. cube2 is a child of cube1, both have translate and rotate values.
        When using the button on cube2, the translate and rotate values are zeroed out (cube2's offset is lost)
        Eg. cube3 child of cube2 child of cube1
        If button function is used on cube3 and then cube2, the zeroed group of cube3 will be the same zero position of cube2's zeroed group.
        Instead it should cube3's zeroed group should be on cube2 and not change!
        ...
        rcl[
            "Options"] = "import tool_panel;import maya_ui.top_group as ui;tool_panel.PopOff(widget=ui.AddTopGroup_Widget)"
        button = tool_panel.IconButton(icon="rig/rig_zeroed_group.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.nodes as rig_nodes;rig_nodes.create_offset_cns()",
                                       hotkey_label="Create Top Group",  # No hotkey, but needed for help button
    ...
    def _button_control_color(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: changes the color of the selected control
        right-click: copies the color of the seleted control (first control is the color you want, second control is the one you're copying to)
        ...
        button = tool_panel.IconButton(icon="rig/rig_color.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import maya_ui.color_tone as cui;cct = cui.CurveColorTone_UI();cct.ui()",
                                       hotkey_label="Control Color",  # No hotkey, but needed for help button
    ...
    def _button_map_controls(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button and runs AIEControlMapping_Widget (class in current .py file) which creates a dictionary of original control names to new control names
        then saves the dictionary to a file (not sure what kind of file...?) in the scene's Maya folder
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_control_mapping.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, AIEControlMapping_Widget),
                                       hotkey_label="Map Names",  # No hotkey, but needed for help button
                                       tool_tip="Map Names\nSet up mapping of control names for animation to pick up in Import/Export",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.purple,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_mirror_controls(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button which mirrors selected controls (control names must have 'L_' and 'R_')
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_control_mirror.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.control as i_control;i_control.mirror_sel()",
                                       hotkey_label="Mirror Curves",  # No hotkey, but needed for help button
    ...
    def _widget_control_slider(self, layout):
        # type: (classmethod) -> None
        """

        #NEEDS REWORK
        Runs _widget_control_slider_run, which is in this class
        Scales EVERY CONTROL created through _button_controls, regardless if it is selected
        Currently scales control's cvs using xform

        INSTEAD:
        should only scale the selected controls and change the control's scale values NOT THE CVS values!

        :param layout: (QHBoxLayout) creates a horizontal box
        :return: None
        """
        # Main Layout
        lyt = QVBoxLayout()
    ...
    # def _widget_control_slider_get_current_size(self, control=None):
    #     bb = cmds.exactWorldBoundingBox(control.name)
    #     control_bb = [(bb[0] - bb[3]) * -1, (bb[1] - bb[4]) * -1, (bb[2] - bb[5]) * -1]
    #
    #     current_scale = [0, 0, 0]
    #     for i, c_bb in enumerate(control_bb):
    #         if c_bb > 0:
    #             current_scale[i] = float(c_bb) / 2.0
    #
    ...
    def _widget_control_slider_run(self):
        # type: (None) -> None
        """
        resizes all controls created with the _buttons_controls function
        :return: None
        """
        # Get controls and if any were selected
        controls_resizing = i_control.get_controls(use_selection=True, info_connections_only=False, dialog_error=False)
    ...
    def _button_multi_channelbox(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a ui that allows user to view up to 3 objects' channel boxes
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_multi_attr.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import maya_ui.multi_cb as mui;mcb = mui.MultiChannelBox_UI();mcb.ui()",
                                       hotkey_label="Multi CB",  # No hotkey, but needed for help button
    ...
    def _button_lock_unlock_attrs(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        # NEEDS REWORK
        Creates a button with left and right click functionality
        left-click: locks all attributes of selected objects
        right-click: locks/unlocks and/or hides all attributes of selected objects

        instead, button should lock/unlock selected attributes, NOT ALL attributes
        NOTE: TSRV attributes means 'T'ranslate, 'S'cale, 'R'otate, and 'V'isibility
        --- does this mean that unlocking attributes using this tool is supposed to unlock the translate, scale, rotate and visibility regardless if they're selected or not?
        ...
        rcl[
            "Lock & Hide (selected attributes)"] = "import icon_api.attr as i_attr;i_attr.lock_and_hide_sel(lock=True, hide=True)"
        rcl[
            "Unlock (TRSV attributes for selected nodes)"] = "import icon_api.attr as i_attr;i_attr.lock_and_hide_sel(unlock=True)"
        rcl[
            "Unlock & Unhide (TRSV attributes for selected nodes)"] = "import icon_api.attr as i_attr;i_attr.lock_and_hide_sel(unlock=True, unhide=True)"
        button = tool_panel.IconButton(icon="rig/rig_lock_unlock.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.lock_and_hide_sel(lock=True)",
                                       hotkey_label="Lock and Hide",  # No hotkey, but needed for help button
                                       tool_tip="Lock and Hide Attributes.\nLeft-click: Lock selected attributes.\nRight-click: Other options",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.yellow,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_quick_vis(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click:
        --- one object selected: adds a 'vis' attribute to selected object which controls object's visibility
        --- two or more objects seleted: adds a 'vis' attribute to first object which controls the visibility of the rest of the selected objects
        right-click: create a dictionary of functions that turns the 'vis' attribute on/off or sets the gimbal's visibility attribute on/off

        #NOTE: right-click functionality does not work
        ...
        """
        rcl = collections.OrderedDict()
        rcl["All Vis - ON"] = "import rig_tools.utils.controls as rig_controls;rig_controls.set_all_vis_attrs(set=1)"
        rcl["All Vis - OFF"] = "import rig_tools.utils.controls as rig_controls;rig_controls.set_all_vis_attrs(set=0)"
        rcl["All Gimbals - ON"] = "import rig_tools.utils.controls as rig_controls;rig_controls.set_gimbal_vis(set=1)"
        rcl["All Gimbals - OFF"] = "import rig_tools.utils.controls as rig_controls;rig_controls.set_gimbal_vis(set=0)"
        button = tool_panel.IconButton(icon="rig/rig_vis_attr.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.create_vis_attr_sel()",
                                       hotkey_label="Quick Vis",  # No hotkey, but needed for help button
    ...
    def _button_attr_up(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button that moves an attribute up in the channel box editor (User-made attributes ONLY)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_arrow_up.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.reorder_sel(direction='up')",
                                       hotkey_label="Attr Up",  # No hotkey, but needed for help button
    ...
    def _button_attr_down(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button that moves an attribute down in the channel box editor (User-made attributes ONLY)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_arrow_down.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.reorder_sel(direction='down')",
                                       hotkey_label="Attr Down",  # No hotkey, but needed for help button
                                       tool_tip="Attr Down.\nMove selected channelBox attribute(s) down.",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.red,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_update_attrs(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button that creates a dictionary of the selected object's current attributes and their values
        (found in Extra Attributes)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_update_attrs.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.default_attrs_sel(update=True)",
                                       hotkey_label="Update Defaults",  # No hotkey, but needed for help button
    ...
    def _button_reset_attrs(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button that resets the object to it's default (zeroed out) values
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_reset_attrs.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.attr as i_attr;i_attr.default_attrs_sel(update=False)",
                                       hotkey_label="Reset to Defaults",  # No hotkey, but needed for help button
    ...
    def _button_follow_attr(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: behaves like a space switcher (except user needs to select a minimum of 3 objects)
        (1) object, creates a 'follow' attribute on selected object and creates a condition node
        --- additional condition nodes are created depending on how many drivers are selected
        (2) object, object to be driven based on the 'follow' attribute of (1)
        (3) object(s), driver(s) of the 'follow' attribute. Creates a parent constraint from the driver(s) to the selected object of (2)

        right-click: Deletes the 'follow' condition node
        --- can only delete one condition node at a time, must select object from (1) then one driver from (3)

        #NEEDS REWORK: does not delete parent constrain from the driver(s) to the selected object of (2)
        NOTE: needs a 'Utility_Grp' transform group to exist before using this button
        ...
        button = tool_panel.IconButton(icon="rig/rig_psb.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.attributes as rig_attributes;rig_attributes.create_follow_attr_sel()",
                                       hotkey_label="Set Up Follow Attr",  # No hotkey, but needed for help button
    ...
    def _button_promote_attr(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button and runs PromoteAttr_Widget which makes a list of the selected
        attributes and 'transfers'/'promotes'/'copies' them to another object

        # 'Promote' is currently not working
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_promote_attr.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, PromoteAttr_Widget),
                                       hotkey_label="Promote Attribute",  # No hotkey, but needed for help button
                                       tool_tip="Promote Attribute\nPromote defined attribute(s) onto selected objects",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.teal,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_dynamics(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: runs DynamicSettings_Widget (class in current .py file) which creates a cloth/collider/hair dynamic depending on choices selected in the UI
        right-click: create a dictionary of dynamic options from the rig_dynamics module or the groom_io module
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Cloth - Assign as 'SkinForce'"] = "import rig_tools.utils.dynamics as rig_dynamics;rig_dynamics.assign_cloth_dupe(as_type='SkinForce')"
        rcl[
            "Cloth - Assign as 'ClothSimMesh'"] = "import rig_tools.utils.dynamics as rig_dynamics;rig_dynamics.assign_cloth_dupe(as_type='ClothSimMesh')"
        rcl["Dynamic Settings UI"] = partial(tool_panel.PopOff, None, DynamicSettings_Widget)
        rcl[
            "Attach Yeti Hair"] = "import assets;from groom_tools import groom_io;groom_io.reconnect_yeti_input(asset=assets.current())"
        button = tool_panel.IconButton(icon="rig/rig_dynamics.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, CreateDynamics_Widget),
                                       hotkey_label="Create Dynamics",  # No hotkey, but needed for help button
    ...
    def _button_paint_dynamics(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: Does not create anything in the scene. Brings up a pop-window stating button only has right-click functionality
        right-click: create a dictionary of dynamic map painting options from the rig_dynamics module (eg. damp, drag, mass, etc.)
        ...
        for opt in map_options:
            nn = logic_py.nice_name_from_camel_case(opt)  # Need to do this method instead of just title()
            rcl[nn] = "import rig_tools.utils.dynamics as rig_dynamics;rig_dynamics.paint_dynamics_sel(map='%s')" % opt
        button = tool_panel.IconButton(icon="rig/rig_paint.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.dynamics as rig_dynamics;rig_dynamics.paint_dynamics_sel()",
                                       hotkey_label="Paint Dynamics",  # No hotkey, but needed for help button
    ...
    def _button_dynamics_weights_import_export(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Runs DynamicsWeightsImportExport_Widget (class in current .py file) that can import or export dynamic weights
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/out_assemblyReference.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, DynamicsWeightsImportExport_Widget),
                                       hotkey_label="Dynamics Weight Import Export",
                                       # No hotkey, but needed for help button
    ...
    def _button_interactive_playback(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Runs 'Interactive Playback' mode (switch to FX Mode, Field/Solvers > InteractivePlayback)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_interactive_playback.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import maya.mel as mel;mel.eval('InteractivePlayback')",
                                       hotkey_label="Interactive Playback",  # No hotkey, but needed for help button
    ...
    def _button_geometry_color(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a button with left and right click functionality
        left-click: creates a lambert/phong texture of the selected color and replaces the texture of the selected object
        right-click: create a dictionary of geometry color options from tex_utils module and rig_geometry module

        NOTE: 'EAV: Make color Dupes' are an EAV process only (http://pipedocs.icon.local:8000/resources/tool/rig_ios/#eav-mesh-tone)
        ...
        button = tool_panel.IconButton(icon="rig/rig_color.png",
                                       ...
                                       hotkey_label="Geo Color",  # No hotkey, but needed for help button
    ...
    def _button_geometry_overrides(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Turns off 'Enable Override' for all geo in the scene, object's Attribute Editor > Object Display > Drawing Overrides > Enable Override
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_geo_overrides.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import tex_utils;tex_utils.disable_drawing_overrides()",
                                       hotkey_label="Geo Overrides",  # No hotkey, but needed for help button
    ...
    def _button_import_references(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        reimports references one by one so the latest mesh is in the same

        # NEEDS REWORK: Currently not working as intended
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_imp_ref.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.geometry as rig_geometry;rig_geometry.import_to_reference_sel()",
                                       hotkey_label="Import to Reference",  # No hotkey, but needed for help button
    ...
    def _button_proxy_geo(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right click functionality
        left-click: creates a polyCylinder on each bind joint (joint name includes '_Bnd_Jnt')
        --- a transform group called 'LowRes_Geo_Grp' must exist to create proxy_geo
        right-click: create a dictionary to modules in the rig_geometry directory
        --- to use 'Constrained Proxy' option, a transform group called 'LowRes_Geo_Grp' must exist to create proxy_geo

        NOTE: right-click 'Cosntrained Proxy' is the same as left-click action
        ...
        button = tool_panel.IconButton(icon="rig/rig_proxy_geo.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.geometry as rig_geometry;rig_geometry.create_proxy_simple()",
                                       hotkey_label="Proxy Geo",  # No hotkey, but needed for help button
    ...
    def _button_joint(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right click functionality
        left-click: takes soft selection of the geo's vertices and applies them as skin weights for the selected joint
        --- rig must be skinned before using this button
        right-click: create a dictionary of joint and skinning options from the deformer director and several classes in this .py file
        --- to use the 'Apply Soft Sel as Weights (sel jnt and skinned verts)' option, rig must be skinned before using this button
        --- JointInsertReskin_Widget, JointsFromComponents_Widget, FkFromEdges_Widget

        NOTE: 'Apply Soft Sel as Weights (sel jnt and skinned verts)' and left click perform the same functionality
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Apply Soft Sel as Weights (sel jnt and skinned verts)"] = "import icon_api.deformer as i_deformer;i_deformer.save_soft_selection()"
        rcl["Insert and Reskin (UI)"] = partial(tool_panel.PopOff, None, JointInsertReskin_Widget)
        rcl["Create Joints on Components (UI)"] = partial(tool_panel.PopOff, None, JointsFromComponents_Widget)
        rcl["FK Chain from Components (UI)"] = partial(tool_panel.PopOff, None, FkFromEdges_Widget)
        button = tool_panel.IconButton(icon="rig/rig_insert_reskin.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.deformer as i_deformer;i_deformer.save_soft_selection()",
                                       hotkey_label="Joint Tools",  # No hotkey, but needed for help button
    ...
    def _button_skinning(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        create a button that opens a ui with buttons with a variety of skinning options
        NOTE: There are lots of skinning options!
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_skinning.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, SkinningWidget),
                                       hotkey_label="Skinning Tools",  # No hotkey, but needed for help button
    ...
    def _button_skinning_tools(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        rework of _button_skinning. Called "Skinning Tools 2.0". Uses text buttons instead of graphic buttons
        NOTE: Lots of skinning options! Must compare with _button_skinning functionality
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Dockable UI"] = partial(tool_panel.dock_widget, SkinningToolsWidget)
        button = tool_panel.IconButton(icon="rig/rig_skinning.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, SkinningToolsWidget),
                                       hotkey_label="Skinning Tools 2.0",  # No hotkey, but needed for help button
                                       tool_tip="Skinning Tools 2.0",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.orange,
                                       no_padding=True,
    ...
    def _button_mirror_joint(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        mirrors selected joints according XY, YZ, XZ and either by behaviour or by orientation
        ...
        rcl[
            "Force Mirror Joint Orient (1:1)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=True)"
        rcl[
            "Force Mirror Joint Orient (0x, -z)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, axis_mult=[0, 1, -1])"
        rcl[
            "Force Mirror Joint Orient (-180 x)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis='x')"
        rcl[
            "Force Mirror Joint Orient (-180 y)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis='y')"
        rcl[
            "Force Mirror Joint Orient (-180 z)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis='z')"
        rcl[
            "Force Mirror Joint Orient (-180 xy)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis=['x', 'y'])"
        rcl[
            "Force Mirror Joint Orient (-180 xz)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis=['x', 'z'])"
        rcl[
            "Force Mirror Joint Orient (-180 yz)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis=['y', 'z'])"
        rcl[
            "Force Mirror Joint Orient (-180 xyz)"] = "import rig_tools.utils.joints as rig_joints;rig_joints.force_mirror_jo(exact=False, sub_axis=['x', 'y', 'z'])"
        button = tool_panel.IconButton(icon="rig/rig_mirror_joint.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, MirrorJoint_Widget),
                                       hotkey_label="Mirror Joint",  # No hotkey, but needed for help button
    ...
    def _button_select_bind_joints(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Checks and selects joints with '_Bnd_Jnt' in it's name
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_selection.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.joints as rig_joints;rig_joints.select_bind_joints()",
                                       hotkey_label="Select Bind Joints",  # No hotkey, but needed for help button
    ...
    def _button_fix_geometry(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right click functionality
        left-click: Renames the shape to match it's transform's name. Currently not working as intended.
        right-click: create a dictionary of 'geometry fixes' to the modules in the rig_geometry and deformer directory

        # NEEDS REWORK:
        --- should be 'fix_deformed_geometry_names' NOT 'fix_geometry'
        --- not working as intended? Could not re-create the situation that this button would fix.
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Fix 'Shape' and 'ShapeOrig' Names"] = "import icon_api.deformer as i_deformer;i_deformer.fix_deform_shape_name_sel()"
        rcl[
            "Swap Helper Poly for Nurbs"] = "import rig_tools.utils.geometry as rig_geometry;rig_geometry.swap_helpers_sel()"
        rcl[
            "Load Reference Check"] = "import rig_tools.utils.geometry as rig_geometry;rig_geometry.reference_check('load')"
        rcl[
            "Remove Reference Check"] = "import rig_tools.utils.geometry as rig_geometry;rig_geometry.reference_check('remove')"
        button = tool_panel.IconButton(icon="rig/rig_fix_shape_names.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.deformer as i_deformer;i_deformer.fix_deform_shape_name_sel()",
                                       hotkey_label="Fix Geometry",  # No hotkey, but needed for help button
    ...
    def _button_box_lattice(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Creates a box lattice  on the selected geo
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_box.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.deformers as rig_deformers;rig_deformers.create_box_lattice_sel()",
                                       hotkey_label="Box Lattice",  # No hotkey, but needed for help button
    ...
    def _button_create_deformer(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Runs CreateDeformer_Widget (in current .py file) which opens a ui with a dropdown menu of deformers
        and a textbox for the deformer name
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_create_deformer.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, CreateDeformer_Widget),
                                       hotkey_label="Create Deformers",  # No hotkey, but needed for help button
    ...
    def _button_paint_softcluster(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Runs the paint attributes tool on the selected geo
        ...
        button = tool_panel.IconButton(icon="rig/rig_paint.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.deformer as i_deformer;i_deformer.paint_cluster()",
                                       hotkey_label="Paint Soft Cluster",
                                       tool_tip="Shortcut to paint tool for selected control's soft cluster.",
                                       right_click_hotkey=True,
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.purple,
                                       no_padding=True,
    ...
    def _button_multi_smooth(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        # NEEDS REWORK:
        Currently opens a ui to run the selected paint operation (in the Paint Attributes Tool) a # of times.
        --- Eg. In the paint attributes tool, if "replace" is the selected paint operation and the user chooses to
        run the "Multi-smooth" tool 5 times, the "replace" paint operation will run 5 time son the selected geo
        --- Instead, should be 'Smooth'
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="anm/43Smooth.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import tool_panel;import maya_ui.multi_smooth as ui;tool_panel.PopOff(widget=ui.MultiSmooth_UI)",
                                       hotkey_label="Multi-Smooth",
    ...
    def _button_follicles(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right click functionality
        left-click: runs ConveyorBelt_Widget which opens a ui that creates a follicle system on the selected geo
        right-click: create a dictionary of follicle creation options to the modules in the rig_deformer directory

        NOTE: left-click and 'Converyor Belt' perform the same functionality
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Conveyor Belt"] = partial(tool_panel.PopOff, None, ConveyorBelt_Widget)
        rcl[
            "Slider - Make a surface and follicle from 2 selected edges."] = "import rig_tools.utils.deformers as rig_deformers;rig_deformers.follicle_slider_sel()"
        rcl[
            "Attach - Attach targets to driver. Select targets then driver."] = "import rig_tools.utils.deformers as rig_deformers;rig_deformers.follicle_attach_sel()"
        button = tool_panel.IconButton(icon="rig/rig_follicles.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, ConveyorBelt_Widget),
                                       hotkey_label="Follicle Tools",  # No hotkey, but needed for help button
                                       tool_tip="Follicle Tools\nLeft-Click: Create Conveyor Belt\nRight-Click: Other options",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.pink,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_at_center(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right functionality
        left-click: creates a joint at the center of the selected objects
        right-click: create a dictionary of component creation options (on the selected objects) from the modules
        in the i_node directory
        ...
        """
        rcl = collections.OrderedDict()
        rcl["-- Locator --"] = self.divider_fn
        rcl[
            "Locator at collective center"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='locator')"
        rcl[
            "Locator at each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='locator', parent_under=False, average_center=False)"
        rcl[
            "Locator under each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='locator', average_center=False)"
        rcl[
            "Locator at collective volume"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='locator', volume_center=True, average_center=False)"
        rcl["-- Joint --"] = self.divider_fn
        rcl[
            "Joint at collective center"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='joint')"
        rcl[
            "Joint at each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='joint', parent_under=False, average_center=False)"
        rcl[
            "Joint under each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='joint', average_center=False)"
        rcl[
            "Joint at collective volume"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='joint', volume_center=True, average_center=False)"
        rcl["-- Transform --"] = self.divider_fn
        rcl[
            "Transform at collective center"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='transform')"
        rcl[
            "Transform at each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='transform', parent_under=False, average_center=False)"
        rcl[
            "Transform under each selected"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='transform', average_center=False)"
        rcl[
            "Transform at collective volume"] = "import icon_api.node as i_node;i_node.build_at_center_sel(build_type='transform', volume_center=True, average_center=False)"
        button = tool_panel.IconButton(icon="rig/rig_at_center.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.node as i_node;i_node.build_at_center_sel(build_type='joint')",
                                       hotkey_label="Build At Center",  # No hotkey, but needed for help button
    ...
    def _button_apply_angle(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Needs at least 2 selected objects, object to get angle from and an object to act as an 'origin point'

        (1) selected object, point to calculate the average angle of from the 'origin point' (the second selected object)
        (2) selected object, acts as an 'origin point' of the angle
        (3) (optional), applies the average rotation to this object, otherwise creates a locator and applies
        rotation to this locator

        NOTE: needs to be renamed (apply angle of rotation) or change the tool tip info to match the actual
        functionality of this tool (https://en.wikipedia.org/wiki/Angle_of_rotation)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_apply_angle.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.utils as i_utils;i_utils.apply_average_rotation_sel()",
                                       hotkey_label="Apply Angle",  # No hotkey, but needed for help button
    ...
    def _button_snap(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right functionality
        left-click: Snaps all selected to the world position of the first selected object
        right-click: create a dictionary of snap options from the modules in the node directory
        --- 'T'ranslate, 'R'otate, 'S'cale,
        --- 'JO' Joint orientation

        # NEED REWORK: 'Radius' and 'TRS and JO, Radius'
        eg. 'Radius'
        --- Joint objects, changes the joint orientation of the object, instead it should change ONLY the
        radius size of the joint
        --- Mesh objects, changes the scale and orientation of the object, instead it should give an error stating
        'selected objects must be joints'
        --- Mesh and Joint objects, changes the scale and orientation of the object, instead, it should give an error
        stating 'selected objects must be joints'
        ...
        button = tool_panel.IconButton(icon="rig/rig_snap.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.node as i_node;i_node.copy_pose_sel(action='snap', attrs='tr')",
                                       hotkey_label="Snap Position",  # No hotkey, but needed for help button
    ...
    def _button_connect(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right functionality
        left-click: connects the first object's translate xyz and rotate xyz attributes to the rest of
        the selected objects' translate xyz and rotate xyz
        right-click: create a dictionary of connect attribute options from the node directory
        --- 'T'ranslate, 'R'otate, 'S'cale
        ...
        button = tool_panel.IconButton(icon="rig/rig_connect.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.node as i_node;i_node.copy_pose_sel(action='connect', attrs='tr')",
                                       hotkey_label="Connect TRS",  # No hotkey, but needed for help button
    ...
    def _button_constrain(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Create a button with left and right functionality
        left-click: runs Constraints_Widget (in current .py file) which opens a ui that lets the user to:
        --- select a constraint type
        --- turn maintain offset on or off
        --- if there is a single driver or multiple drivers
        right-click: create a dictionary of constraint options for multiple drivers (maintain offset on or off) from the
        modules in the node directory
        ...
        """
        rcl = collections.OrderedDict()
        rcl["-- Maintain Offset ON (Multi-Driver) --"] = self.divider_fn
        rcl[
            "TR (multi, on)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='tr', mo=True)"
        rcl[
            "T (multi, on)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='t', mo=True)"
        rcl[
            "R (multi, on)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='r', mo=True)"
        rcl[
            "S (multi, on)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='s', mo=True)"
        rcl[
            "TRS (multi, on)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='m', mo=True)"
        rcl["-- Maintain Offset OFF (Multi-Driver) --"] = self.divider_fn
        rcl[
            "TR (multi, off)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='tr', mo=False)"
        rcl[
            "T (multi, off)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='t', mo=False)"
        rcl[
            "R (multi, off)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='r', mo=False)"
        rcl[
            "S (multi, off)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='s', mo=False)"
        rcl[
            "TRS (multi, off)"] = "import icon_api.node as i_node;i_node.copy_pose_sel(action='constrain', attrs='m', mo=False)"
        button = tool_panel.IconButton(icon="rig/rig_constrain.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command=partial(tool_panel.PopOff, None, Constraints_Widget),
                                       hotkey_label="Constrain TRS",  # No hotkey, but needed for help button
    ...
    def _button_mirror_objects(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Mirrors selected objects by the translateX attirubte
        ...
        button = tool_panel.IconButton(icon="rig/rig_control_mirror.png",
                                       ...
                                       hotkey_label="Mirror Objects",  # No hotkey, but needed for help button
    ...
    def _button_find_rigs(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        create a button with left and right functionality
        left-click: runs find_rigs_update() from the rig_tools directory
        --- opens a confirmDialog window that confirms if user wants to find rigs that need updating, process
        takes a long time, esp if scene is big
        --- creates a .json file on the desktop. Contains a string list of rigs that need updating
        right-click: runs FindRigsToUpdate_Widget (in current .py file) which opens a ui to enter a file path to save
        the .json file
        --- default: creates a anim folder on the desktop and saves .json file in folder
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Define File Path"] = partial(tool_panel.PopOff, None, FindRigsToUpdate_Widget)
        button = tool_panel.IconButton(icon="rig/rig_find_rigs.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.procs.find_rigs_to_update as find_rigs;find_rigs.find_rigs_update()",
                                       hotkey_label="Find Rigs",  # No hotkey, but needed for help button
                                       tool_tip="Find rigs that need to be updated and display the information.",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.white,
                                       no_padding=True,
                                       )
        self._add_button(layout, button, row, column)

    def _button_rename(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        runs Rename_Widget (in current .py file) which opens a ui that renames the selected objects
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_search_replace.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import tool_panel;import maya_ui.rename as ui;tool_panel.PopOff(widget=ui.Rename_Widget)",
                                       hotkey_label="Rename Nodes",  # No hotkey, but needed for help button
    ...
    def _button_add_remove_sets(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        create a button with left and right functionality
        left-click: creates a new objectSet called 'newSet'
        --- will add selected objects if the set name is not changed
        right-click: create a dictionary of set member options from the modules in the icon_api.utils directory

        # NEEDS REWORK
        'Define Set Name (Instead of Selecting)' right-click option doesn't work
        --- "sets_member_edit() got an unexpected keyword argument 'add' "
        ...
        rcl = collections.OrderedDict()
        rcl[
            "Add to Set (Select Set Last)"] = "import icon_api.utils as i_utils;i_utils.sets_member_edit_sel(add_objs=True)"
        rcl[
            "Remove from Set (Select Set Last)"] = "import icon_api.utils as i_utils;i_utils.sets_member_edit_sel(add_objs=False)"
        rcl[
            "Define Set Name (Instead of Selecting)"] = "import tool_panel;import maya_ui.sets_edit as ui;tool_panel.PopOff(widget=ui.SetMemberEdit_Widget)"
        button = tool_panel.IconButton(icon="rig/rig_sets.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import icon_api.utils as i_utils;i_utils.sets_member_edit_sel(add_objs=True)",
                                       hotkey_label="Set Member Edit",  # No hotkey, but needed for help button
    ...
    def _button_fix_misc(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        create a button with left and right click functionality
        left-click: matches the selected objs' shape name to reflect the objs' name
        right-click: create a dictionary of object renaming fixes from the modules in the icon_api .utils, .node,
        and .constraint directory

        # NEEDS REWORK
        --- 'Fix Constraint Target Names' not working. When a constraint is selected, the function gives
        an error "No object matches the name: <constraint_name>"
        --- Not sure if 'Clear Stored Nodes/Attrs' is working. Clears all icon_apu lists
        --- but does it clear non-icon_api lists?
        ...
        rcl[
            "Fix Constraint Target Names"] = "import icon_api.constraint as i_constraint;i_constraint.rename_constraint_target_attrs_sel()"
        rcl["Clear Stored Nodes/Attrs"] = "import icon_api.utils as i_utils;i_utils.reset_iconapi_scene_data()"
        ...
        button = tool_panel.IconButton(icon="rig/rig_fix_shape_names.png",
                                       ...
                                       hotkey_label="Fix Random Things",  # No hotkey, but needed for help button
    ...
    def _button_import_calethestics_test_misc(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        NOTE: Calesthestics test == ROM test

        create a button with left and right click functionality
        left-click: deletes all animation and imports calesthestics test animation that is suitable for the
        asset in the scene
        --- requires a rig to be in the scene
        right-click: create a dictionary of calisthenics options from the modules in the calethestics_test directory
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Set Timeline to Clip"] = "import rig_tools.procs.import_calethestics_test as calTest;reload(calTest);calTest._set_full_frame_range()"
        rcl["-- Remove --"] = self.divider_fn
        rcl[
            "Remove Animation from Test"] = "import rig_tools.procs.import_calethestics_test as calTest;reload(calTest);calTest._remove_animation(cmds.ls(sl=True))"
        rcl["-- PAIE --"] = self.divider_fn
        rcl["Load PAIE UI"] = "import anm_tools.utils.paie as paie;paie.GUI()"
        rcl["-- IO Pose --"] = self.divider_fn
        rcl["Export All Controls Attribute Values"] = "import rig_tools.utils.attributes as jsonAttr;jsonAttr.export_attributes_values_entire_scene()"
        rcl[
            "Import All Controls Attribute Values"] = "import rig_tools.utils.attributes as jsonAttr;jsonAttr.import_attributes_values()"
        button = tool_panel.IconButton(icon="anm/22StudioLibrary.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.procs.import_calethestics_test as calTest;reload(calTest);calTest._import_paie(cmds.ls(sl=True))",
                                       hotkey_label="Imports the animation calethestics test.",  # No hotkey, but needed for help button
                                       tool_tip="If your scene has the animation controls just press button.",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.gold,
                                       no_padding=True,
    ...
    def _flo_three(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        NOTE: Currently WIP (PAX Rigging_framework)
        ...
        """
        rcl = collections.OrderedDict()
        rcl["Launch all"] = "import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch()"
        rcl["Launch Face Builder"] = "import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch_face_widget()"
        rcl["Launch Deformer Builder"] = "import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch_deformer_widget()"
        rcl["Launch Rigging Wizard"] = "import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch_workflow_widget()"
        rcl["Launch Rig Builder"] = "import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch_rig_widget()"
        button = tool_panel.IconButton(icon="rig/meta_cube.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rigging_widgets.widget_launchers.launch_widgets as lw;lw.launch()",
                                       hotkey_label="",
                                       tool_tip="",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
    ...
    def _button_set_smooth(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Not sure how this tool works.
        NOTE: Not working? Is it supposed to set the "displaySmoothness" of an object?
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_set_smooth.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.geometry as rig_geometry;rig_geometry.set_smooth()",
                                       hotkey_label="Set Smoothing",  # No hotkey, but needed for help button
    ...
    def _button_bone_on(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Changes the draw style of all joints to "Joint" (Attribute Editor > Joint > Draw Style > Bone)
        ...
        """
        rcl = collections.OrderedDict()
        button = tool_panel.IconButton(icon="rig/rig_bone_onoff.png",
                                       overlay_image=self.rcl_overlay if rcl else None,
                                       command="import rig_tools.utils.joints as rig_joints;rig_joints.bone_vis(1)",
                                       hotkey_label="Bone On",  # No hotkey, but needed for help button
                                       tool_tip="Bone On",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.green,
    ...
    def _button_bone_off(self, layout, row, column):
        # type: (classmethod, int, int) -> None
        """
        Changes the draw style of all joints to "None" (Attribute Editor > Joint > Draw Style > Bone)
        ...
        button = tool_panel.IconButton(icon="rig/rig_bone_onoff.png",
                                       ...
                                       hotkey_label="Bone Off",  # No hotkey, but needed for help button
                                       tool_tip="Bone Off",
                                       right_click_buttons=rcl,
                                       resize=self.button_size,
                                       icon_rgb=tool_panel.OverlayColors.red,
...
class CreateDynamics_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        # type: (classmethod) -> None
        """
        Initializes the default values and the UI

        :param parent: (QMainWindow) - creates the UI window
        """
        ...
        # Add dynamic type dropdown
        self.type_dropdown = QComboBox()

        # Add solver dropdown
        # solver_layout = QHBoxLayout()
        self.solver_dropdown = QComboBox()
        # solver_sel_btn = QPushButton("Sel")
        # solver_sel_btn.clicked.connect(partial(ui_utils.load_sel_in_textfield, self.hsm_text, False))
        # solver_sel_btn.setToolTip(str("Choose selected solver."))
        # solver_layout.addWidget(solver_sel_btn)

        # Add button
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        self.button = tool_panel.IconButton(icon="rig/rig_create.png", command=partial(self.run),
    ...
    def _add_dynamics_options(self):
        # type: () -> None
        """
        Adds a list of strings to the drop down menu
    ...
    def _add_solver_options(self, default=None):
        # type: (str) -> None
        """
        Adds the solver name to the drop down menu

        :param default: (str) - string of the solver option
        :return: None
        """
    ...
    def _tt_change(self):
        # type: () -> None
        """
        Changes the tool tip text depending on which dynamic type is selected
    ...
    def run(self, *args):
        """
        Gets the user's input from the UI and passes the data to CreateDynamics
...
class ConveyorBelt_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add Button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create Conveyor Belt",
    ...
    def run(self, *args):
        """
        Gets the user's input from the UI and passes the data to create_conveyor_belt()
...
class JointInsertReskin_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add checkbox to update skinning
        ...
        # Add Button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Insert Joints", command=partial(self.run),
    ...
    def run(self):
        # type: () -> None
        """
        Gets the user's input from the UI and passes the data to insert_joints_reskin() or insert_joints()
...
class PromoteAttr_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add select button - to load attribute
        ...
        # Add run button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Promote Attr", command=partial(self.run),
    ...
    def _load_attribute(self, *args):
        """
        Loads selected channel box attributes into the UI's text box
    ...
    def run(self, *args):
        """
        Gets the user's input from the UI and passes the data to transfer_attributes()
...
class AIEControlMapping_Widget(qt_utils.MainWindow):
    def __init__(self, parent=qt_utils.get_main_window()):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        save_button = tool_panel.IconButton(icon="rig/rig_create.png", label="Save Control Map",
    ...
    def add_lines(self, num=15):
        # type: (int) -> None
        """
        Adds <num> rows in two columns with default text of "Control Name"

        :param num: (int) - number of rows to initialize in the UI
        :return: None
        """
        if num == 0:
            return
    ...
    def add_more_lines(self):
        # type: () -> None
        """
        Adds additional lines to the UI, depending on the user's input
    ...
    def save(self):
        # type: () -> None
        """
        Takes the user's input from the UI and converts into a dictionary and saves the information as a file
...
class SaveControlShape_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add run button
        button = tool_panel.IconButton(icon="rig/rig_save_shape.png", label="Save Control Shape",
                                       command=partial(self.run),
                                       tool_tip="Save Selected transforms as a Control Shape Type", resize=[30, 30],
                                       icon_rgb=tool_panel.OverlayColors.green)
        layout.addWidget(button)
    ...
    def run(self, *args):
        """
        Gets the user's input from the UI then passes the user input and selected curves to save_curve_as_type()
...
class ReplaceControlShape_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Replace Control Shape",
    ...
    def run(self):
        # type: () -> None
        """
        Gets the user's input from the UI and passes the data to name_prompt() or replace_shape()
...
def load_sel_line_edit(sel_kws=None, line_edit_obj=None):
    # type: (dict, classmethod) -> None
    """
    Converts the selection (dictionary) into a list and sets the text in the text box as the list

    :param sel_kws: (dict) - dictionary of selected components
    :param line_edit_obj: - (QLineEdit) text input field
    :return: None
    """
    # Query selection
    if not sel_kws:
...
def create_sel_line_edit_objs(default_text=None, text_tt=None, button_tt=None, sel_kws=None):
    # type: (str, str, str, dict) -> [classmethod, classmethod, classmethod]
    """
    Creates the default layout for the UI input field and button and takes the selected objects and passes
    them to load_sel_line_edit()

    :param default_text: (str) - default text in the QLineEdit input field
    :param text_tt: (str) - QLineEdit's input field tool tip (appears when hovering over the input field)
    :param button_tt: (str) - QPushButton's tool tip (appears when hovering over the button)
    :param sel_kws:(dict) - dictionary of selected components
    :return: (list) - default UI values for the layout, input field and button
    """
    # Layout
    layout = QHBoxLayout()
...
def create_name_layout(tooltip=None, side_cc=None, default_text=None):
    # type: (str, classmethod, str) -> [classmethod, classmethod, classmethod]
    """
    Create default layout for the UI drop down and input field and adds a dictionary of "side" values
    to the drop down menu

    :param tooltip: (str) - QLineEdit's tool tip (appears when hovering over the input field)
    :param side_cc: (str) - if not None, QComboBox will connect with the side's control color
    :param default_text: (str) - default text in the QLineEdit input field
    :return: (list) - default UI values for the layout, drop down menu and input field
    """
    # Layout
    layout = QHBoxLayout()
    if not tooltip:
...
def create_control_objs(type_cc=None, add_default_color_opt=False):
    # type: (classmethod, bool) -> [classmethod, classmethod, classmethod]
    """
    Create a default layout for the UI drop down and adds a dictionary of "control"
    and "color" values to the drop down menus

    :param type_cc: (classmethod) - UI check box possibilities based on the user's choice in the drop down menu
    :param add_default_color_opt: (bool) - creates a 'default' option in the drop down menu
    :return: (list) - default UI values for the layout and drop down menus
    """
    layout = QHBoxLayout()
...
class FkFromEdges_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create Fk Chains", command=partial(self.run),
    ...
    def run(self):
        # type: () -> None
        """
        Gets the user's input from the UI and passes the data to fk_chain_from_edges()
...
class JointsFromComponents_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create Joints", command=partial(self.run),
    ...
    def run(self):
        # type: () -> None
        """
        Gets the user's input from the UI and passes the data to joints_from_components()
...
class CreateControl_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add run button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create Control", command=partial(self.run),
                                       tool_tip="Create Control using specified information")
        layout_end.addWidget(button)

        # Add to window
        # self.main_layout.addWidget(QLabel("Create Control"))
        # self.main_layout.addLayout(layout_name)
    ...
    def _disable_option(self, option_widget=None):
        ...
        :param option_widget: (QCheckBox) - the checkbox to disable
        :return: None
        """
    ...
    def _restore_option(self, option_widget=None, state=None):
        # type: (classmethod, classmethod) -> None
        """
        Sets the state of the checkbox

        :param option_widget: (QCheckBox) - the checkbox to restore
        :param state: (Qt.Checked) - the state to restore the check box to (checkable or disabled)
        :return: None
        """
    ...
    def _side_color(self):
        # type: () -> None
        """
        Create a dictionary of default "side" values for the UI dropdown menu
...
class DynamicsWeightsImportExport_Widget(qt_utils.MainWindow):
    ...
    def _add_widgets(self):
        self.pthText = QLineEdit('{0}'.format(os.environ["HOMESHARE"]))
    ...
    def _export_csv_pre(self):
        pathStr = self.pthText.text().replace('\\', '/')
        os.mkdir(pathStr)
        if len(cmds.ls(sl=True)) and self.pthText.text():
            self._export_csv(cmds.ls(sl=True)[0], pathStr)
    ...
    def _import_csv(self, selItems, filePath):
        if not os.path.isfile(filePath):
            ...
            # assemble deformerList and indexList
            deformerList = []
            indexList = []  # list, two items = 1st(int):index to fetch in csv file 2nd: the vtx name to set the value
...
class CreateDeformer_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add dropdown for which type of deformer
        self.type_dropdown = QComboBox()
        ...
        # Add Button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create\nDeformer", command=partial(self.run),
...
class IO_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None, default_io=None):
        ...
        # - Button
        import_button = tool_panel.IconButton(icon="rig/rig_import.png", icon_rgb=tool_panel.OverlayColors.yellow,
                                              resize=[30, 30],
                                              command=partial(self.run, "import"), tool_tip="Import IO",
                                              hotkey_label="Import IO")
        ...
        # - Button
        export_button = tool_panel.IconButton(icon="rig/rig_export.png", icon_rgb=tool_panel.OverlayColors.green,
                                              resize=[30, 30],
                                              command=partial(self.run, "export"), tool_tip="Export IO",
                                              hotkey_label="Export IO")
    ...
    def multi_opts(self, action):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        button = tool_panel.IconButton(icon="rig/rig_create.png", hotkey_label="Multi IO",
                                       command=partial(self.multi_run, action))
        button_layout.addWidget(button)
    ...
    def multi_run(self, action):
        ...
        if "Controls - All Info" in run_io_types:
            if "Controls - Colors Only" in run_io_types or "Controls - Shapes Only" in run_io_types:
                ...
                return
        # # - Duplicates?
        # core_types = [display.split(" - ")[0] for display in run_io_types]
        # too_many = []
        # for typ in core_types:
        #     if core_types.count(typ) > 1:
        #         too_many.append(typ)
        # if too_many:
        #     i_utils.error("Can only choose 1 of duplicate types: %s" % ", ".join(too_many), dialog=True)
        #     return
...
class FindRigsToUpdate_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Add button
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Find Rigs",
...
class MirrorJoint_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        do_button = tool_panel.IconButton(icon="rig/rig_create.png", label="Mirror Joint", command=partial(self.run))
...
class DynamicSettings_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        # Load button
        load_button = tool_panel.IconButton(label="Load Dynamics", icon=os.path.join(image_path, 'passsquare.png'),
                                            command=partial(self.load_setting))
        button_layout.addWidget(load_button)

        if as_dev:
            # Save button
            save_button = tool_panel.IconButton(label="Save Dynamics",
                                                icon=os.path.join(image_path, 'warningsquare.png'),
                                                command=partial(self.save_setting))
            button_layout.addWidget(save_button)

            # Delete button
            del_button = tool_panel.IconButton(label="Delete Dynamics",
                                               icon=os.path.join(image_path, 'errorsquare.png'),
...
class SkinningWidget(qt_utils.MainWindow):
    def __init__(self, parent=qt_utils.get_main_window()):
        super(SkinningWidget, self).__init__(parent)
        ...
        cw_button = tool_panel.IconButton(label="Copy Skins", command=partial(self._copy_skins),
                                          icon="anm/19IKFKMatch.png",
                                          tool_tip="Copy Skins from first selected to rest selected", resize=[30, 30],
                                          icon_rgb=tool_panel.OverlayColors.blue_dark,
        ...
        cpw_store_button = tool_panel.IconButton(label="Store", command=partial(self._store_weights),
                                                 icon="rig/rig_import.png", icon_rgb=tool_panel.OverlayColors.yellow,
                                                 tool_tip="Stores weights of selected verts", resize=[30, 30],
                                                 # right_click_buttons=cpw_rcl, overlay_image="rig/rig_overlay_rightclick.png"
                                                 )

        cpw_avg_button = tool_panel.IconButton(label="Store\nAverage", command=partial(self._store_weights, True),
                                               icon="rig/rig_import.png", icon_rgb=tool_panel.OverlayColors.purple,
                                               tool_tip="Stores averaged weights of selected verts", resize=[30, 30])

        cpw_paste_button = tool_panel.IconButton(label="Paste", command=partial(self._paste_weights),
                                                 icon="rig/rig_export.png", icon_rgb=tool_panel.OverlayColors.green,
                                                 tool_tip="Pastes stored weight values onto selected verts",
                                                 resize=[30, 30])

        io_ui_button = tool_panel.IconButton(icon="rig/rig_io_import_export.png",
                                             command=partial(self._open_io_ui), label="Imp/Exp",
                                             hotkey_label="Import Export",  # No hotkey, but needed for help button
                                             tool_tip="Import and Export Data", resize=[30, 30],
                                             icon_rgb=tool_panel.OverlayColors.orange)
        ...
        # - Multi-Smooth
        multi_smooth_button = tool_panel.IconButton(label="Multi\nSmooth",
                                                    command="import tool_panel;import maya_ui.multi_smooth as ui;tool_panel.PopOff(widget=ui.MultiSmooth_UI)",
        ...
        soft_sel_weights_button = tool_panel.IconButton(label="Soft Sel\nWeights",
                                                        command="import icon_api.deformer as i_deformer;i_deformer.save_soft_selection()",
        ...
        # - Force Normal
        normal_button = tool_panel.IconButton(label="Normalize",
        ...
        # - Kill RGB
        kill_rgb_button = tool_panel.IconButton(label="Kill RGB\nConnections", command=partial(self._kill_rgb),
    ...
    def _open_io_ui(self):
        tool_panel.PopOff(widget=IO_Widget, help_url="http://pipedocs.icon.local:8000/resources/tool/rig_ios/",
                          default_io="Deformer - skinCluster - Mesh Only")
...
class SkinningToolsWidget(qt_utils.MainWindow):
    def __init__(self, parent=qt_utils.get_main_window()):
        super(SkinningToolsWidget, self).__init__(parent)
        ...
        # Smooth Weights
        cw_label_layout = QHBoxLayout()
        cw_label = QLabel("Smooth Weights")
        cw_label.setFont(bold_font)
        cw_label_layout.addWidget(cw_label)
        cw_label_layout.setAlignment(Qt.AlignCenter)

        smooth_sel_layout = QVBoxLayout()

        # # Smooth Selected
        cw_layout = QHBoxLayout()

        cpw_label = QLabel("Opacity:")
        cpw_label.setMaximumWidth(40)
        cw_layout.addWidget(cpw_label)

        smooth_opacity = QDoubleSpinBox()
        smooth_opacity.setMaximum(1.0)
        smooth_opacity.setMinimum(0.0)
        smooth_opacity.setDecimals(2)
        smooth_opacity.setValue(1.0)
        smooth_opacity.setMaximumWidth(60)
        smooth_opacity.setSingleStep(0.1)
        cw_layout.addWidget(smooth_opacity)

        smooth_selected = QPushButton("Smooth Selected")
        smooth_selected.setMinimumWidth(50)
        smooth_selected.setMinimumHeight(30)
        smooth_selected.clicked.connect(partial(i_deformer.smooth_skin_selected_call, smooth_opacity))
        cw_layout.addWidget(smooth_selected)

        # # Paint Smooth Tool
        smooth_paint = QPushButton("Paint Smooth Tool")
        smooth_paint.setMinimumWidth(80)
        smooth_paint.setMinimumHeight(50)
        smooth_paint.clicked.connect(partial(i_deformer.set_tool, smooth_opacity))
        smooth_sel_layout.addWidget(smooth_paint)

        # Common Tools
        # # Label
        cpw_label_layout = QHBoxLayout()
        cpw_label = QLabel("Common Tools")
        cpw_label.setFont(bold_font)
        cpw_label_layout.addWidget(cpw_label)
        cpw_label_layout.setAlignment(Qt.AlignCenter)
        ...
        eq_layout.setAlignment(Qt.AlignCenter)

        eq_vts = QPushButton("Equalize\nVerticies")
        eq_vts.setMinimumWidth(70)
        eq_vts.setMinimumHeight(15)
        eq_vts.clicked.connect(i_deformer.equalize_call)
        eq_layout.addWidget(eq_vts)

        eq_edges = QPushButton("Equalize\nEdges (Loop)")
        eq_edges.setMinimumWidth(70)
        eq_edges.setMinimumHeight(15)
        eq_edges.clicked.connect(partial(i_deformer.equalize_call, True))
        eq_layout.addWidget(eq_edges)

        vtx_copy = QPushButton("Vertex\nCopy")
        vtx_copy.setMinimumWidth(70)
        vtx_copy.setMinimumHeight(15)
        vtx_copy.clicked.connect(partial(self._store_weights, True))
        eq_layout.addWidget(vtx_copy)

        vtx_paste = QPushButton("Vertex\nPaste")
        vtx_paste.setMinimumWidth(70)
        vtx_paste.setMinimumHeight(15)
        vtx_paste.clicked.connect(partial(self._paste_weights))
        eq_layout.addWidget(vtx_paste)

        # # Row B Buttons
        eqB_layout = QHBoxLayout()
        eqB_layout.setAlignment(Qt.AlignCenter)

        normalize_button = QPushButton("Normalize")
        normalize_button.setMinimumWidth(115)
        normalize_button.clicked.connect(partial(i_deformer.force_normalize_skins))
        eqB_layout.addWidget(normalize_button)

        hammer_button = QPushButton("Hammer")
        hammer_button.setMinimumWidth(115)
        hammer_button.clicked.connect(partial(self._hammer_weights))
        eqB_layout.addWidget(hammer_button)

        add_bind_joint = QPushButton("+")
        add_bind_joint.setMinimumWidth(15)
        add_bind_joint.setToolTip('Add bind joints without reassigning the skinning.')
        add_bind_joint.clicked.connect(self._add_influence)
        eqB_layout.addWidget(add_bind_joint)

        add_bind_joint = QPushButton("-")
        add_bind_joint.setMinimumWidth(15)
        add_bind_joint.setToolTip('Removes influence from select mesh, or deletes skin cluster completely if no joints are selected.')
        add_bind_joint.clicked.connect(self._remove_influence)
        eqB_layout.addWidget(add_bind_joint)

        # # copy weights
        copywgt_layout = QHBoxLayout()
        copywgt_layout.setAlignment(Qt.AlignCenter)

        copy_skins = QPushButton("Copy Skins")
        copy_skins.setMinimumWidth(320)
        copy_skins.setMinimumHeight(35)
        copy_skins.setToolTip('Copy weights - Select the source object last.')
        copy_skins.clicked.connect(self._copy_skins_simple)
        copywgt_layout.addWidget(copy_skins)

        # Painting Tools
        # - Label
        paint_label_layout = QHBoxLayout()
        paint_label = QLabel("Import/Export Skin data")
        paint_label.setFont(bold_font)
        paint_label_layout.addWidget(paint_label)
        paint_label_layout.setAlignment(Qt.AlignCenter)
        ...
        import_export_layout.setAlignment(Qt.AlignCenter)

        import_skin = QPushButton("Import Skin")
        import_skin.setMinimumWidth(150)
        import_skin.setMinimumHeight(15)
        import_skin.clicked.connect(self._load_skin)
        import_export_layout.addWidget(import_skin)

        export_skin = QPushButton("Export Skin")
        export_skin.setMinimumWidth(150)
        export_skin.setMinimumHeight(15)
        export_skin.clicked.connect(self._save_skin)
        import_export_layout.addWidget(export_skin)

        import_export_path_layout = QHBoxLayout()
        import_export_path_layout.setAlignment(Qt.AlignCenter)

        self.pthText = QLineEdit(skin_tools.SKIN_PATH)
        import_export_path_layout.addWidget(self.pthText)

        browse_path = QPushButton("...")
        browse_path.clicked.connect(self._browse_path_skin)
        import_export_path_layout.addWidget(browse_path)
        ...
        # - Copy Skins
        self.main_layout.addLayout(cw_label_layout)
        self.main_layout.addLayout(smooth_sel_layout)
        smooth_sel_layout.addLayout(cw_layout)
        # - SEP
        self.main_layout.addWidget(separator())
        # - Copy/Paste/Average Weights
        self.main_layout.addLayout(cpw_label_layout)
        self.main_layout.addLayout(eq_layout)
        self.main_layout.addLayout(eqB_layout)
        self.main_layout.addLayout(copywgt_layout)
        # - SEP
        self.main_layout.addWidget(separator())
        # - Paint
        self.main_layout.addLayout(paint_label_layout)
        self.main_layout.addLayout(import_export_layout)
        self.main_layout.addLayout(import_export_path_layout)
        # - SEP
        self.main_layout.addWidget(separator())
        # - Misc Skin Tools
        self.main_layout.addLayout(misc_label_layout)
    ...
    def _copy_skins_simple(self):
        i_deformer.copy_skin_sel(force=True, rui=True)
    ...
    def _hammer_weights(self):
        mel.eval('weightHammerVerts')

    def _add_influence(self):
        selList = cmds.ls(sl=True)
        i_deformer.add_bind_joints_call(selList)

    def _remove_influence(self):
        selList = cmds.ls(sl=True)
        i_deformer.remove_bind_joints_call(selList)
    ...
    def _save_skin(self):
        import skin_tools
        skin_tools.save_skin(filesDirectory=self.pthText.text())

    def _load_skin(self):
        import skin_tools
        skin_tools.load_skin(filesDirectory=self.pthText.text())

    def _browse_path_skin(self):
        dialog = QFileDialog.getExistingDirectory(self, caption="Browse", dir=self.pthText.text())
        self.pthText.setText(dialog)
...
class BlendControls_Widget(qt_utils.MainWindow):
    def __init__(self, parent=qt_utils.get_main_window()):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        button = tool_panel.IconButton(icon="rig/rig_create.png", label="Create", command=partial(self.run))
        button_layout.addWidget(button)
...
class FaceBake_Widget(qt_utils.MainWindow):
    def __init__(self, parent=qt_utils.get_main_window()):
        ...
        btn = tool_panel.IconButton(label="Bake", command=partial(self.run), icon="rig/rig_cake.png",
...
class Constraints_Widget(qt_utils.MainWindow):
    def __init__(self, parent=None):
        ...
        button_layout = QHBoxLayout()
        button_layout.setAlignment(Qt.AlignCenter)
        button = tool_panel.IconButton(icon="rig/rig_constrain.png", icon_rgb=tool_panel.OverlayColors.orange,
                                       hotkey_label="Constrain",
                                       command=partial(self.run), resize=[30, 30])
        button_layout.addWidget(button)
        self.main_layout.addLayout(button_layout)
