...
def do_it():
    ...
    print("----- EYE PROJECTION SETUP COMPLETE!")

def build():

    # Check for eyes
    eyes = []
    nodes = cmds.listRelatives("HiRes_Geo_Grp", ad=1, type="transform")
    for n in nodes:
        if "R_Eye_Geo" in n:
            eyes.append(n)
        if "L_Eye_Geo" in n:
    ...
    iris_color_dict = {"blue": (0, 0.6, 0.85), "brown": (0.22, 0.1, 0), "green": (0, 0.675, 0), "gold": (0.65, 0.55, 0),
                       "purple": (0.35, 0.15, 0.65), "pink": (1, 0.25, 1), "grey": (0.1, 0.1, .1)}
    iris_color = None
    ...
    for side in ["L", "R"]:
        ...
        for e in eyes:
            ...
            cmds.skinCluster('Head_Bnd', e, toSelectedBones=True)
    ...
    cmds.parentConstraint("Head_Bnd", "EyeProjectionsPivot_Grp", maintainOffset=True)
    cmds.scaleConstraint("Head_Bnd", "EyeProjectionsPivot_Grp", maintainOffset=True)

    lPlane = cmds.nurbsPlane( name="L_Eye_Proj_Fol_Geo", degree=1, patchesU=1, patchesV=1, axis=(0, 0, 1) )[0]
    ...
    tempCon = cmds.parentConstraint("L_Eye_Bnd", lPlane, maintainOffset=False)
    cmds.delete(tempCon)
    cmds.parent(lPlane, projGrp)

    rPlane = cmds.nurbsPlane( name="R_Eye_Proj_Fol_Geo", degree=1, patchesU=1, patchesV=1, axis=(0, 0, 1) )[0]
    ...
    tempCon = cmds.parentConstraint("R_Eye_Bnd", rPlane, maintainOffset=False)
    cmds.delete(tempCon)
    ...
    rev = cmds.shadingNode("multiplyDivide", asUtility=True, name="smoEyesProj_Pos_RevX_Mult")
    ...
    tempCon = cmds.pointConstraint("L_Eye_Bnd", "L_Eye_Proj_Jnt", maintainOffset=False)
    cmds.delete(tempCon)
...
def add_deformers(geometry, squash=True, bendX=True, bendY=True):
	# add deformers if they exists
	if squash == True:
		if cmds.objExists("HeadSquash_Y1"):
				cmds.nonLinear("HeadSquash_Y1", e=1, g=geometry, foc=1)

	if bendX == True:
		if cmds.objExists("HeadBend_X1"):
				cmds.nonLinear("HeadBend_X1", e=1, g=geometry, foc=1)
			
	if bendY == True:
		if cmds.objExists("HeadBend_Z1"):
				cmds.nonLinear("HeadBend_Z1", e=1, g=geometry, foc=1)


def connect():

    # smo projection eyes bind

    if cmds.objExists("smoEyesProj_Pos_RevX_Mult"):
        jnt_matrix = cmds.xform("R_Eye_Proj_Jnt", q=True, ws=True, m=True)
        cmds.delete("smoEyesProj_Pos_RevX_Mult")
    ...
    if cmds.objExists("EyeProjections_Grp"):
        for side in ['L_', 'R_']:
            ...
            if cmds.objExists("HeadSquash_Y1"):
				cmds.nonLinear("HeadSquash_Y1", e=1, g=side + 'Eye_Proj_Fol_Geo')

            cmds.skinCluster(side+'Eye_Bnd', side + "Eye_Proj_Fol_Geo", toSelectedBones=True)
        ...
        cmds.select("Eye_Root_Ctrl")
    else:
        cmds.confirmDialog(message="Eyes not built yet.")

    # # bottom of chain!!!
    # for side in "LR":
    #     #add_deformers(side + '_Eye_Proj_Fol_Geo', bendX=False, bendY=False)
    #     if cmds.objExists("HeadSquash_Y1"):
	# 			cmds.nonLinear("HeadSquash_Y1", e=1, g=side + '_Eye_Proj_Fol_Geo')
...
def create_proxy_eyes():
    """Create the proxy eyes by duplicating the geo and baking a texture. """

    proxy_eye_grp = cmds.group(n="Proxy_Eye_Geo_Grp", em=1, p="HiRes_Geo_Grp")
    proxy_eye_utils = cmds.group(n="Proxy_Eye_Utl_Grp", em=1, p="Utility_Grp")
    proxy_eye_deform_utils = cmds.group(n="Proxy_Eye_Deform_Grp", em=1, p="Proxy_Eye_Utl_Grp")
    eye_geo = cmds.listRelatives("Eye_Geo_Grp")
    cmds.hide(proxy_eye_utils)

    for i in eye_geo:
        geo = i.split(":")[1]
        side = geo[:1]
        
        save_path = _create_proxy_tex_path(i)
        proxy_tex = cmds.convertSolidTx(side + "_Eye_Projection.outColor", i, rx=256, ry=256, fileFormat="png", fileImageName=save_path + "/" + side + "_Eye_loRes_Shdr.png", n=side + "_eye_Proj")

        cmds.setAttr(proxy_tex[0] + ".ignoreColorSpaceFileRules", 0)
        cmds.setAttr(proxy_tex[0] + ".colorSpace", "lnh", type="string")
        
        shader = cmds.shadingNode('lambert', n=side + "_eyeProxy_Shdr", asShader=True )
        proxy_set = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name=side + "_eyeProxy_ShdrSG")
        
        cmds.connectAttr(proxy_tex[0] + ".outColor",  shader + ".color", f=1)
        cmds.connectAttr(shader + ".outColor", proxy_set + ".surfaceShader.")
        
        proxy_geo = cmds.duplicate(i, n="Proxy_" + geo)[0]
        cmds.sets(proxy_geo, e=True, forceElement=proxy_set)
        cmds.parent(proxy_geo, proxy_eye_grp)
        
        # create proxy eye functionality
        output_geo = cmds.duplicate(i, n=proxy_geo.replace("Geo", "Output"))[0]
        deform_geo = cmds.duplicate(i, n=proxy_geo.replace("Geo", "Deform"))[0]
        cmds.parent(output_geo, deform_geo, proxy_eye_utils)

        # add on squash deformers 
        add_deformers(output_geo)
        
        shrink = cmds.deformer(output_geo, type="shrinkWrap", n=output_geo.replace("Output", "Swp"), foc=1)
        clst = cmds.cluster(output_geo, n=output_geo.replace("Output", "Clr"), foc=1)
        clst_grp = cmds.group(n=output_geo.replace("Output", "Cluster_Grp"), em=1)
        cmds.setAttr(shrink[0] + ".projection", 4)
        cmds.connectAttr(deform_geo + "Shape " + ".worldMesh[0]", output_geo.replace("Output", "Swp") + ".targetGeom")

        # orient cluster properly to eye joint then connect up directly
        ori = cmds.xform(side + "_Eye_Bnd", q=1, ro=1)
        cmds.xform(clst_grp, ro=ori)
        cmds.parent(clst[1], clst_grp)
        cmds.connectAttr(side + "_Eye_Bnd.r", clst[1] + ".r")

        cmds.parent(clst_grp, deform_geo, proxy_eye_deform_utils)
        
        bsp = cmds.blendShape(output_geo, proxy_geo, n=proxy_geo + "_BSH", foc=1)[0]
        cmds.setAttr(bsp + "." + output_geo, 1)
        
        cmds.skinCluster("Head_Bnd", proxy_geo, n=proxy_geo + "_SC", tsb=True, mi=5, dr=4.0)

    cmds.addAttr("Eye_Root_Ctrl", ln="TextureResolution", at="enum", en="Proxy:Texture", k=1)
    rev = cmds.createNode("reverse", n="Eye_Root_Rev")
    cmds.connectAttr("Eye_Root_Ctrl.TextureResolution", rev + ".inputX")
    cmds.connectAttr("Eye_Root_Ctrl.TextureResolution", "Eye_Geo_Grp.v")
    cmds.connectAttr(rev + ".outputX", proxy_eye_grp + ".v")

def _create_proxy_tex_path(item):

    # asset_split = item.split("_")
    # asset_name = ("_").join([asset_split[0], asset_split[1]])

    colon_split = item.split(":")
    name_space_join = colon_split[0].split("_")[:-1]
    asset_name = ("_").join(name_space_join)

    path = "Y:/SMO/assets/type/Character/" + asset_name + "/work/elems/rig_data/eyeProxy"
