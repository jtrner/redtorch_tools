...
def do_it():
    ...
    if cmds.window('totsEyes', exists=True):
        cmds.deleteUI('totsEyes')

    window = cmds.window("totsEyes", title="TOTS Eye Projections", iconName='totsEyes', widthHeight=(200, 140))

    cmds.columnLayout("totsEyesLayout", adjustableColumn=True)
    ...
    if cmds.objExists("EyeProjections_Grp"):
        cmds.button("conButt", enable=True, parent="totsEyesLayout", edit=True)
...
def build(self):
    ...
    rev = cmds.shadingNode("multiplyDivide", asUtility=True, name="totsEyesProj_Pos_RevX_Mult")
...
def connect(self):
    # tots projection eyes bind

    if cmds.objExists("totsEyesProj_Pos_RevX_Mult"):
        jnt_matrix = cmds.xform("R_Eye_Proj_Jnt", q=True, ws=True, m=True)
        cmds.delete("totsEyesProj_Pos_RevX_Mult")
