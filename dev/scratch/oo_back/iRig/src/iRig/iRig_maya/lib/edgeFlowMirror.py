        ...
        if self.task.startswith('geometry'):
            ...
            if redo:
                for i in range(targetPointArray.length()):
                    targetPointArray.set(i, self.newTargetPoints[pointIndex].x, self.newTargetPoints[pointIndex].y,
                                         self.newTargetPoints[pointIndex].z)
                    pointIndex += 1

            else:
                for i in range(targetPointArray.length()):
                    targetPointArray.set(i, self.oldTargetPoints[pointIndex].x, self.oldTargetPoints[pointIndex].y,
            ...
            if foundSkin:
                ...
                for i in range(self.infCount):
                    ...
                    if jointNames[i].split(":")[-1].startswith(searchString):
                        searchJointNames.append(
                            nameSpace + jointNames[i].split(':')[-1][len(searchString):len(jointNames[i])])
                        searchJointIndexes.append(i)
                    elif jointNames[i].split(":")[-1].startswith(replaceString):
                        replaceJointNames.append(
                            nameSpace + jointNames[i].split(':')[-1][len(replaceString):len(jointNames[i])]);
                ...
                for i in range(bothIndexes.length()):
                    ...
                    #
                        ...
                        for k in range(self.infCount):
                            averageWeights[k] = (self.WeightArray[startIndexB + k] + self.WeightArray[
                                startIndexB + jointMapArray[k]]) * 0.5;
                        for k in range(self.infCount):
        ...
        if self.task == "geometryFlip" or self.task == "geometryMirror" or self.task == "geometrySymmetry":  # geometry mirror
            ...
            for i in range(targetPoints.length()):
                self.oldTargetPoints.append(
                    OpenMaya.MFloatVector(targetPoints[i].x, targetPoints[i].y, targetPoints[i].z))
            ...
            if mirrorInt != 2:  # if it's not create symmetryshape
                if not doVertexSpace:
                    ...
                    for i in range(len(bothIndexes)):
                        if mirrorInt == 0 or direction == sideArray[bothIndexes[i]]:
                            ...
                            self.newTargetPoints.set(
                                OpenMaya.MFloatVector(basePoints[mapArray[bothIndexes[i]]].x - pointOffset.x,
                                                      basePoints[mapArray[bothIndexes[i]]].y + pointOffset.y,
                                                      basePoints[mapArray[bothIndexes[i]]].z + pointOffset.z),
                    ...
                    for i in range(len(bothIndexes)):
                        ...
                        if (basePoints[bIndex] - targetPoints[bIndex]).length() < 0.00001 and (
                                basePoints[mapArray[bIndex]] - targetPoints[mapArray[bIndex]]).length() < 0.00001:
                            doSkipThisOne = True
                            ...
                            if mirrorInt == 0 or direction == sideArray[bIndex] or sideArray[bIndex] == 0:
                                ...
                                for k in range(idsCount):
                                    ...
                                    baseX += (OpenMaya.MVector(
                                        basePoints[xIds[bIndex][k]] - basePoints[bIndex])) * xMult
                                    baseZ += (OpenMaya.MVector(
                                        basePoints[zIds[bIndex][k]] - basePoints[bIndex])) * zMult
                                ...
                                baseMat = createMatrixFromList(
                                    [baseX.x, baseX.y, baseX.z, 0, baseNormals[bIndex].x, baseNormals[bIndex].y,
                                     baseNormals[bIndex].z, 0, baseZ.x, baseZ.y, baseZ.z, 0, basePoints[bIndex].x,
                                     basePoints[bIndex].y, basePoints[bIndex].z, 1])
                                ...
                                if sideArray[bIndex] == 0:  # middle vertex
                                    ...
                                    targetMat = createMatrixFromList(
                                        [targetX.x, targetX.y, targetX.z, 0, baseNormals[mappedBIndex].x,
                                         baseNormals[mappedBIndex].y, baseNormals[mappedBIndex].z, 0, targetZ.x,
                                         targetZ.y, targetZ.z, 0, basePoints[mappedBIndex].x,
                                         basePoints[mappedBIndex].y, basePoints[mappedBIndex].z, 1])

                                    changetarget = changeLocal * targetMat

                                self.newTargetPoints.set(
                                    OpenMaya.MFloatVector(changetarget(3, 0), changetarget(3, 1), changetarget(3, 2)),
                                    mappedBIndex)
                ...
                for i in range(bothIndexes.length()):
                    if mapArray[bothIndexes[i]] != -1:
                        ...
                        self.newTargetPoints.set(
                            OpenMaya.MFloatVector(targetPoints[bothIndexes[i]].x, targetPoints[bothIndexes[i]].y,
