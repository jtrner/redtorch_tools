...
pluginPath = os.path.join(thisDir, '../plugin/scripted/replaceInfluence.py')
pluginPath = os.path.abspath(pluginPath)
if not mc.pluginInfo('replaceInfluence', q=True, loaded=True):
...
def blend_fk_ik(fk_joint_list, ik_joint_list, result_joint_list, blendAttr):
    ...
    for fk, ik, rsl in zip(fk_joint_list[1:], ik_joint_list[1:], result_joint_list[1:]):
        ...
        for attr in ("translate",):
            bln = mc.createNode("blendColors", n=rsl + attr.title() + "_BLN")
            rev = mc.createNode("reverse", n=rsl + attr.title() + "_REV")
        ...
        # blend rotate
        pbl = mc.createNode("pairBlend", n=rsl + "Rotate_PBL")
    ...
    for fk, ik, rsl in zip(fk_joint_list, ik_joint_list, result_joint_list):
        bln = mc.createNode("blendColors", n=rsl + "Scale_BLN")
        rev = mc.createNode("reverse", n=rsl + "Scale_REV")
        mc.connectAttr(blendAttr, rev + ".inputX")
        mc.connectAttr(rev + ".outputX", bln + ".blender")
...
def extract_from_to(from_this="", to_this="", search="_JNT", replace="Rsl_JNT"):
    ...
    for j in duplicated_list:
        ...
        name = renameLib.removeEndNumbers(name)
        j = mc.rename(j, name)
        result.append(j)
...
def create_on_curve(curve=None, numOfJoints=5, parent=True, description='C_base'):
    ...
    curve_dup = mc.duplicate(curve)[0]
    mc.rebuildCurve(curve_dup, ch=False, rpo=True, rt=0, end=1, kr=0, kcp=False, kep=True, kt=0, s=200, d=1, tol=0.01)
    curveShape = curveLib.getShapes(curve_dup)[0]
