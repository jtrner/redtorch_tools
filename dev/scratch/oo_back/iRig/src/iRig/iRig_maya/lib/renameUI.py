...
Usage:
from iRig_maya.lib import renameUI
reload(renameUI)
renameUI.launch()
"""
...
class UI(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def removeEndNumber(self):
        ...
        renameLib.removeEndNumber(objs=mc.ls(sl=True))
    ...
    def rename(self):
        ...
        if name:
            renameLib.rename(objs=mc.ls(sl=True), name=name, pad=pad, startNum=startNum)
        ...
        renameLib.renameReplace(objs=mc.ls(sl=True), search=search, replace=replace)
        ...
        renameLib.renameAdd(objs=mc.ls(sl=True), prefix=prefix, suffix=suffix)
