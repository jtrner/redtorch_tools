...
def node_plug_walk(node):
    ...
    for attribute in controller.scene.listAttr(node.name):
        if '.' not in attribute:
            yield node.plugs[attribute]
            if controller.scene.attributeQuery(attribute, node=node.name, lc=True) is True:
...
def test_shardMatrix():
    ...
    for node_plug in node_plug_walk(obj_node):
        mc.getAttr(node_plug, type=True)
