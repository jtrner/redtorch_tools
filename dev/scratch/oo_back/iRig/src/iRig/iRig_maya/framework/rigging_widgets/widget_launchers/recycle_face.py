...
def recycle(controller, data_path, selected=False):
...
def duplicate_meshs(meshs, tag):
    ...
    new_meshs = []
    for mesh in meshs:
        duplicate_geo = mc.duplicate(mesh, name='%s_%s' % (tag, mesh.replace('Shape', '')))[0]
        mc.setAttr('%s.v' % duplicate_geo, True)
...
def recycle_blendshape(controller, data, selected=False):
    ...
    if selected:
        reference_geometry = controller.get_selected_mesh_names()
    else:
    ...
    for reference_mesh in reference_geometry:
        ...
        if reference_blendshapes:
            old_blendshapes.extend(reference_blendshapes)
    ...
    #for i in range(len(reference_geometry)):
    #    blendshape = mc.blendShape(base_geometry[i], reference_geometry[i], foc=True)[0]
    #    mc.setAttr('%s.w[0]' % blendshape, 1.0)

    #mc.setAttr('%s.v' % base_group, False)
    controller.face_progress_signal.emit()


def recycle_rig(root, data, selected=False):
    ...
    reference_mesh = reference_geometry[0]
    ...
    for part in root.get_parts():
        ...
        if isinstance(part, Face):
            face_rig = part
    if not jaw_handle:
        raise Exception('Jaw not found.')
    if not jaw_vertices:
        raise Exception('Jaw vertices not found.')
    if not all(jaw_vertices):
        raise Exception('Jaw vertices not found.')
    ...
    for face_group_data in data['face_groups']:
        ...
        if controller.scene.objExists(node_name):
            ...
            face_group = controller.face_network.create_group(
                driver_plug=driver_plug,
                pretty_name=name,
                side=face_group_data.get('side', None),
                root_name=name,
            )
            for driver_value in face_group_data['driver_values']:
                rounded_value = round(driver_value, 3)
