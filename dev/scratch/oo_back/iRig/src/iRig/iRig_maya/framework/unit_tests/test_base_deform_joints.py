...
if __name__ == '__main__':
    ...
    controller.toggle_state()  # Toggle to RIG STATE
    parts = controller.root.get_parts()
    test_reports = list(['\n----- Joints/Base Deform Joints per Part mismatching report -----'])
    for part in parts:
        joints = part.get_joints()
        base_deform_joints = part.get_base_deform_joints()
        if len(joints) != len(base_deform_joints):
            test_str = "# of joints per part mismatch: {0}_{1}, Joints: {2}, Base Deform Joints: {3}".format(
                part.side, part.root_name, joints, base_deform_joints)
            test_reports.append(test_str)
