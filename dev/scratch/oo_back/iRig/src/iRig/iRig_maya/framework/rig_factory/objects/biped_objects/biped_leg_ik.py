...
class BipedLegIkGuide(ChainGuide):
    ...
    def create(cls, controller, **kwargs):
        ...
        this = super(BipedLegIkGuide, cls).create(controller, **kwargs)
        body = this.get_root()
...
class BipedLegIk(Part):
    ...
    def create(cls, controller, **kwargs):
        ...
        this = super(BipedLegIk, cls).create(controller, **kwargs)
        root_name = this.root_name
        size = this.size
        side = this.side
        ...
        for i, joint_str in enumerate(['upper', 'lower', 'ankle', 'toe', 'toe_tip']):
            if i != 0:
        ...
        isolated_joints = [isolate_hip_joint, isolate_knee_joint]
        ...
        ik_joints[2].plugs['scale'].connect_to(joints[2].plugs['scale'])
        ik_joints[2].plugs['scale'].connect_to(joints[2].plugs['scale'])
        root.add_plugs(
            [
        ...
        this.isolated_joints = isolated_joints
        return this
