...
class PostScriptWidget(QFrame):

    done_signal = Signal()

    def __init__(self, *args, **kwargs):
        super(PostScriptWidget, self).__init__(*args, **kwargs)
        font = QFont('', 12, True)
        font.setWeight(100)
        self.vertical_layout = QVBoxLayout(self)
        self.horizontal_layout = QHBoxLayout()
        self.button_layout = QHBoxLayout()
        self.label = QLabel('Post Scripts', self)
        ...
        self.post_script_view = PostScriptView(self)
        ...
        self.vertical_layout.addWidget(self.post_script_view)
        ...
        self.new_button.pressed.connect(self.create_new_post_script)

        self.post_script_view.remove_items_signal.connect(self.remove_items)
        self.controller = None

    def create_new_post_script(self, *args):
        model = self.post_script_view.model()
        if model:
            ...
            if success:
                ...
                if check_valid_name(input_text):
                    script_path = '{0}/{1}.py'.format('%s/post_scripts' % model.controller.build_directory, input_text)
    ...
    def load_model(self):
        model = PostScriptModel()
        model.set_controller(self.controller)
        self.post_script_view.setModel(model)
    ...
    def remove_items(self, indices):
        ...
        for script_name in script_names:
            if script_name in self.controller.root.post_scripts:
                self.controller.root.post_scripts.remove(script_name)
            else:
                self.raise_error(StandardError('Couldnt find script: %s in : %s' % (script_name, self.controller.root.post_scripts)))
        self.load_model()
    ...
    def add_existing(self):
        if self.controller and self.controller.root:
            ...
            post_scripts_directory = '%s/post_scripts' % self.controller.controller.build_directory,
            path, types = QFileDialog.getOpenFileName(
                self,
                'Add Post-Script',
                post_scripts_directory,
                'Python Scripts (*.py)'
            )
            if path:
                if post_scripts_directory not in path:
                    self.raise_error(IOError('You must pick a script in the assets "post_scripts" directory'))
                if os.path.exists(path):
                    base = os.path.basename(path)
                    script_name = os.path.splitext(base)[0]
                    if script_name not in self.controller.root.post_scripts:
                        self.controller.root.post_scripts.append(script_name)
                    self.load_model()
...
class PostScriptView(QListView):
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        script_path = '{0}/{1}.py'.format('%s/post_scripts' % model.controller.build_directory, script_name)
        if not os.path.exists(script_path):
            self.raise_error(StandardError('The post script doesnt exist'))
        launch_action_in_notepad(script_path)
...
class PostScriptModel(QAbstractListModel):

    main_font = QFont('', 12, False)

    def __init__(self):
        super(PostScriptModel, self).__init__()
        self.scripts = []
    ...
    def set_controller(self, controller):
        ...
        if self.controller:
            if isinstance(self.controller.root, ContainerGuide):
                ...
                self.scripts = [x for x in self.controller.root.post_scripts]
