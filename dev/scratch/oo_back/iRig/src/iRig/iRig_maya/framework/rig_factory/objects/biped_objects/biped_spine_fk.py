...
class BipedSpineFkGuide(ChainGuide):
    ...
    def __init__(self, **kwargs):
        super(BipedSpineFkGuide, self).__init__(**kwargs)
    ...
    def create(cls, controller, **kwargs):
        ...
        this = super(BipedSpineFkGuide, cls).create(controller, **kwargs)
        return this

    def get_toggle_blueprint(self):
        blueprint = super(BipedSpineFkGuide, self).get_toggle_blueprint()
...
class BipedSpineFk(Part):
    ...
    def create(cls, controller, **kwargs):
        ...
        this = super(BipedSpineFk, cls).create(controller, **kwargs)
        root = this.get_root()
        size = this.size
        side = this.side
        ...
        for x, matrix in enumerate(matrices):
            ...
            if not x == 0 and not x == len(matrices)-1:
                handle = this.create_handle(
                    handle_type=LocalHandle,
                    size=size*1.25,
                    matrix=Matrix(matrices[x].get_translation()),
                    shape='partial_cube',
