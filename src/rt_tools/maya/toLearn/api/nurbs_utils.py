...
@dec.m_object_arg
def get_curve_cv_positions(nurbs_curve, world_space = False):
    ...
    return [[cv_array[i].x, cv_array[i].y, cv_array[i].z] for i in range(cv_array.length())]
...
def create_text_curve(text, font, parent):
    """
    ...
    """
    root = mc.textCurves(text=text, font=font)[0]
    ...
    for i, curve_name in enumerate(nurbs_curves):
        mc.parent(curve_name, parent, r=True, s=True)
        curve_m_objects.append(get_m_object(curve_name))
