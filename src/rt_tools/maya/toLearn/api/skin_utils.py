...
def __get_skin_blend_weights(o_skin=None):
    """
    ...
    obj_dag_path, obj_component = __get_skin_components(skin_fn=o_skin)
    # procure the blend weight component data
    o_skin.getBlendWeights(obj_dag_path, obj_component, weights_array)
    blend_weights_array = [x for x in weights_array]
    if blend_weights_array:
        all_zro = all(not v for v in blend_weights_array)
        return False if all_zro else blend_weights_array
    else:
        return False


def __set_skin_blend_weights(o_skin=None, blend_weight_data=None):
    """
    ...
    o_skin.setBlendWeights(obj_dag_path, obj_component, m_blendweights)
    return True
...
def get_skin_blend_weights(skin_cls=None):
    ...
    """
    o_skin = get_m_object(skin_cls)
    skin_fn = oma.MFnSkinCluster(o_skin)
    return __get_skin_blend_weights(o_skin=skin_fn)


def set_skin_blend_weights(skinNode, weight_data):
    """
    returns the skin blend weight data.
    :param skin_cls: <class> object.
    :param weight_data: <list> weight values.
    :return: <list> blend weights, <bool> False for nothing.
    """
    o_skin = get_m_object(skinNode)
    skin_fn = oma.MFnSkinCluster(o_skin)
    return __set_skin_blend_weights(o_skin=skin_fn, blend_weight_data=weight_data)
...
def skin_as(skin, target_mesh):
    ...
    skin_name = get_selection_string(skin)

    # get the joints affecting the skin node
    influence_names = mc.skinCluster(skin_name, influence=True,  q=True)

    # bind the target sdk_objects to the source skin and copy the weights to the target object.
    target_mesh_name = get_selection_string(target_mesh)

    target_skin_name = mc.skinCluster(target_mesh_name, influence_names, tsb=1)[0]
    mc.copySkinWeights(
        ss=skin_name,
        ds=target_skin_name,
        noMirror=1,
...
def getWeights(skin_cluster):
    ...
    for shapeName in geometry:
        ...
        wlPlug.getExistingArrayAttributeIndices(indices)
