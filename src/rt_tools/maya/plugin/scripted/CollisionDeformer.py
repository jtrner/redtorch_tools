...
class CollisionDeformer(ompx.MPxDeformerNode):
    ...
    type_id = om.MTypeId(0x0011E195)
    type_name = "CollisionDeformer"
    ...
    def deform(self, data_block,geometry_iterator, local_to_world_matrix, geometry_index):
        ...
        colliderListHandle = data_block.inputArrayValue(CollisionDeformer.colliderList)
...
def initializePlugin(plugin):
    ...
    try:
        plugin_fn.registerNode(
            CollisionDeformer.type_name,
            CollisionDeformer.type_id,
            CollisionDeformer.creator,
            CollisionDeformer.initialize,
            ompx.MPxNode.kDeformerNode
        )
    except:
        print "failed to register node {0}".format(CollisionDeformer.type_name)
        raise
...
def uninitializePlugin(plugin):
    ...
    try:
        plugin_fn.deregisterNode(CollisionDeformer.type_id)
    except:
...
def main():
    ...
    mc.select(mesh)
    deformer_nodes = cmds.deformer( type='CollisionDeformer' )
    mc.connectAttr(
    '{0}.worldMesh'.format(collider_shapes[0]),
    '{0}.CollisionList[0]'.format(deformer_nodes[0]),
    )
main()    
